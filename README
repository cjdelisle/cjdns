...
perhaps for censorship resistance, perhaps network security and I have no doubt you are wondering what the hell this thing is supposed to do.
...
_*A System Is Only Secure When Nobody Has Total Control*_
...
[http://chat.efnet.org:9090/?channels=%23cjdns&Login=Login](http://chat.efnet.org:9090/?channels=%23cjdns&Login=Login).
...
Please read the [Whitepaper], or atleast skim it ;)
...
[http://cjdns.pastebay.org/](http://cjdns.pastebay.org/)
...
Hint2: Might work same under [Ubuntu] and [Linux Mint].
...
_Check to see which libevent is installed:_
...
### 2. Get newest versions of dependencies, some additional tool are required to build cjdns:

        # apt-get install build-essential cmake git

### 3. `libevent2` is required:

CHECK https://github.com/libevent/libevent for LATEST version, this document assumes 2.0.16.

_Grab the stable tarball from libevent and untar_:

        # wget https://github.com/downloads/libevent/libevent/libevent-2.0.16-stable.tar.gz
        # tar -xzf libevent-2.0.16-stable.tar.gz

_Enter directory and compile libevent:_

        # cd libevent-2.0.16-stable
        # ./configure

_Resolve missing dependencies if needed and run again until all errors gone:_
...
_Grab it from GitHub and change to the source directory:_

        # git clone https://github.com/cjdelisle/cjdns.git cjdns
        # cd cjdns

_Setup `build` directory and change to it:_

        # mkdir build
        # cd build

_You Likely want DEBUG logs (this is VERY ALPHA after all), so set the
`Log_LEVEL` environment variable:_

        # export Log_LEVEL=DEBUG

_Pre-build step with `cmake`:_
...
_Run cjdroute without options for HELP:_

      # ./cjdroute

## Step 1:

  _Generate a new configuration file:_
...
## Step 2:
  ...
  _Create a cjdns user so it can run unprivileged._

      # useradd cjdns

  _Create a new TUN device and give the cjdns user authority to access it:_
...
## Step 3: Bootstraping your first connection.
  ...
  **Note: While cjdns is a mesh-network, there isn't auto-discovery of nodes to peer with. You
  will need the key from a friend's node to add your own node to their network.**
...
**Notes:**
  ...
  _To delete a tunnel, use this command:_
...
Once your node is running, you're now a newly minted IPv6 host. Your operating system may 
automatically reconfigure network services to use this new address. If this is not what you 
intend, you should check to see that you are not offering more services then you intended to ;)

_Use `ifconfig -a` to find your tun device's IP address (same as above)_:

_Run `nmap` to discover which services are accessible from this address:_
...
[Whitepaper]: https://github.com/cjdelisle/cjdns/raw/master/rfcs/Whitepaper.txt
