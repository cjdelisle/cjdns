...
В цивилизованном, демократическом обществе каждый должен иметь свободу слова, а цензурироваться могут лишь такие очевидно-плохие вещи как: Детская порнография, мошенничество, спам.
...
Программное обеспечение работает стабильно, но мы не знаем как поведут себя в реальном мире наши алгоритмы и протоколы.  
...
Множество программ которые взаимодействуют с интернетом, не проверяют неизменность пакета, а такие программно-аппаратные средства как DPI, могут вносить значительные изменения в пакеты.
...
В cjdns сети, пакет который идет на локальный (программный) маршрутизатор, помечается, добавляя в него наилучший маршрут для пакета. Лучший маршрут - это самый ближний физически к вам роутер, имеющий адрес близкий по значению с адресом назначения.
...
и у него всё еще нет замены DNS, но имя осталось. Если вы хотите - можете придумать свою расшифровку названия
...
Найти людей которые готовы подключить вас вы можете найти в IRC канале, либо вы можете использовать данные тестового пира http://hype.rusblock.com/Public_peers.html

  * irc://irc.EFNet.org/#cjdns
  * http://chat.efnet.org:9090/?channels=%23cjdns&Login=Login


## 3: Заполните информацию о найденном друге.
...
По любым вопросам вы можете проконсультироваться в IRC, на сервере irc.efnet.org канал - #cjdns
...
Защищайте файл настроек, именно он ваша безопасность в сети, ваш идентификатор. Если у вас его украдут то ваша безопасность будет скомпрометирована, злоумышленник сможет представится вами в сети.
...
Когда cjdroute запущен и работает, интерфейс администратора доступен по адресу localhost:11234 (Обратите внимание, что это не веб интерфейс, веб интерфейс вы можете получить используя nodejs cjdns admin)
