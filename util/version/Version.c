 ...
 */
#define Version_COMPAT(one, ...) [one] = ((int[]){ __VA_ARGS__, -1 }),

static int* Version_COMPATIBILITY[Version_CURRENT_PROTOCOL + 1] = {
...
static inline int isCompat(int smaller, int bigger) {
    int* compat = Version_COMPATIBILITY[bigger];
    for (int i = 0; compat[i] != -1; i++) {
        if (compat[i] == smaller) {
            return true;
        }
    }
...
bool Version_isCompatible(uint32_t one, uint32_t two)
{
    ...
    if (one < two) {
        return isCompat(one, two);
    } else if (two < one) {
        return isCompat(two, one);
    } else {
        return true;
    }
}
