...
void getLuid(const char* name, struct Allocator* alloc, NET_LUID *pout); // CHECKFILES_IGNORE
void getLuid(const char* name, struct Allocator* alloc, NET_LUID *pout)  // CHECKFILES_IGNORE
{
    uint16_t ifName[IF_MAX_STRING_SIZE + 1] = {0};
    MultiByteToWideChar(CP_ACP, 0, name, strlen(name), ifName, IF_MAX_STRING_SIZE + 1);
    ConvertInterfaceAliasToLuid(ifName, pout);

    //WinFail_check(alloc, ConvertInterfaceAliasToLuid(ifName, pout));
    //WinEr_check(alloc, ConvertInterfaceAliasToLuid(ifName, pout));
    //Er_ret();
}
                            ...
                            struct Allocator* tempAlloc))
{
    ...
    getLuid(interfaceName, tempAlloc, &(ipRow.InterfaceLuid));
    ...
    setupRoute(interfaceName, address, prefixLen, addrFam, tempAlloc);
    Er_ret();
}
