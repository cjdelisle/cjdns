...
static enum ArchInfo detectBits()
{
    switch (sizeof(long)) {
...
enum ArchInfo ArchInfo_detect()
{
        ...
        return ArchInfo_Arch_AMD64
            | assertBits(ArchInfo_Bits_64)
            | assertEndian(ArchInfo_Endian_LITTLE);

    #elif defined(__i386__) || defined(__x86__) || defined(__X86__) || \
          defined(_M_IX86) || defined(__i386)

        uint32_t out = ArchInfo_I386
            | assertBits(ArchInfo_Arch_32)
            | assertEndian(ArchInfo_Endian_LITTLE);

        #ifdef __SSE2__
            out |= ArchInfo_I386_SSE2
        #endif

    #elif defined(__ARM_EABI__) || defined(__arm__)

        uint32_t out = ArchInfo_ARM
            | detectBits()
            | detectEndian();

        #ifdef __ARM_EABI__
            out |= ArchInfo_Arm_EABI;
        #endif
        #ifdef __ARM_NEON__
            out |= ArchInfo_Arm_NEON;
        #endif
        ...
        return ArchInfo_PPC
            | assertBits(ArchInfo_Arch_64)
            | assertEndian(ArchInfo_Endian_BIG);
        ...
        uint32_t out = ArchInfo_SPARC | detectBits() | assertEndian(ArchInfo_Endian_BIG);
        switch (ArchInfo_getBits(arch)) {
            case ArchInfo_Arch_32: return out | ArchInfo_Sparc_v8;
            case ArchInfo_Arch_64: return out | ArchInfo_Sparc_v9;
            default return ArchInfo_UNKNOWN;
        }

    #elif defined(__mips__) || defined(__mips) || defined(__MIPS__)

        int out = ArchInfo_MIPS | detectEndian();

        #if defined(_ABIO32)

            return out | assertBits(ArchInfo_Arch_32) | ArchInfo_Mips_OABI;

        #elif defined(_ABIN32)

            return out | assertBits(ArchInfo_Arch_32) | ArchInfo_Mips_NABI;

        #else

            // If it's not oabi or nabi and it's not mips64 then it's UNKNOWN.
            return out | assertBits(ArchInfo_Arch_64);

        #endif

    #endif

    return ArchInfo_UNKNOWN;
}
