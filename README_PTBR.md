...
alocação de endereço e uma tabela de hash distribuída para roteamento. Isso fornece praticamente 
nenhuma necessidade de configuração, e previne problemas de segurança e escalabilidade 
que afligem as redes existentes.
...
* [Hyperboria][] a maior rede de cjdns, a partir de outubro de 2015 existem 2100 nós.
* [/r/darknetplan][]
* [#cjdns on Twitter][]
...
* [Configurar um gateway NAT do cjdns para sua LAN](doc/nat-gateway.md)
* [Instalar cjdns no OpenIndiana](doc/open-indiana.md)

Obrigado por seu tempo e interesse,

Os desenvolvedores do cjnns.

--------------------------------------------------------------------------------

## Como instalar o cjdns

Essas instruções são para distribuições Linux e MacOS baseadas em Debian. Eles deveriam ser
informativo o suficiente para uso em outras distribuições - simplesmente não espere que eles
trabalho textualmente. Se você quer saber o que [a base do sistema operacional vai aqui](https://upload.wikimedia.org/wikipedia/commons/1/1b/Linux_Distribution_Timeline.svg).

### 0. Instalar dependências

Em ambas as plataformas, instalar [Node.js](https://nodejs.org/), embora preferível,
não é estritamente necessário. Se Node.js estiver indisponível ou uma versão inaceitável,
Será baixado e instalado na source.
...
Construindo a partir do pacote:
`sudo yum localinstall https://kojipkgs.fedoraproject.org//packages/cjdns/17.4/4.el6/src/cjdns-17.4-4.el6.src.rpm`

Se você estiver em um laptop e suspender ou hibernar, cjdroute vai demorar alguns
minutos para fazer café e descobrir o que aconteceu quando ele acorda. Você
pode acelerar isso dramaticamente com:

    systemctl enable cjdns-resume

O serviço de retomada reinicia o cjdns quando o sistema acorda.
...
Você pode instalar o cjdns executando
