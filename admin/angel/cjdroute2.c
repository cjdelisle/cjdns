...
static int genconf(struct Random* rand, bool eth)
{
    ...
    printf("    // Dropping permissions.\n"
           ...
           "    // network traffic and/or keys is highly likely.\n"
           "    // The following rules are designed to prevent such an event from also creating\n"
           "    // an in-road for an attacker to further exploit the system on which cjdns is\n"
           "    // running by sheding unnecessary permissions.\n"
           "    // Counter-intuitively, cjdns is *more* secure if it is started as root because\n"
           "    // non-root users do not have permission to use chroot, one of the mitigation\n"
           "    // strategies.\n"
           "    \"security\":\n"
           "    [\n"
           ...
           "        { \"noforks\": 1 },\n"
           "\n"
           "        // Limit the amount of address space accessable to cjdns, this has the effect\n"
           "        // of imposing a hard limit on the amount of memory which cjdns can allocate.\n"
           "        // As the address space size includes cjdns executable and all libraries, the\n"
           "        // actual number is hard-coded internally.\n"
           "        // NOTE: In the event that cjdns retains full root privileges (eg: setuser is\n"
           "        //       disabled), this will fail and cause the engine to be halted during\n"
           "        //       startup.\n"
           "        // Default: enabled\n"
           "        { \"aslimit\": 1 },\n"
           "\n"
