...
struct Linker_x06676152948914862;
...
struct Linker_x0024586421443752116;
# 33 "./util/Assert.h"
__attribute__((__format__ (__printf__, 1, 2)))
...
struct Linker_x05699612782661367;
...
struct Linker_x02300686466240598;
...
# 1 "./util/CString.h" 1
# 21 "./util/CString.h"
struct Linker_x09431693884328403;
...
struct Linker_x009400789344547933;
...
struct Linker_x03437456222764379;
# 34 "./dht/Address.h"
struct Address
{
...
# 1 "./util/log/Log.h" 1
# 20 "./util/log/Log.h"
struct Linker_x026645362922165106;
...
struct Linker_x07217534526265927;
...
# 1 "./benc/List.h" 1
# 21 "./benc/List.h"
struct Linker_x0926618187293172;
...
struct Linker_x048828182140635024;
...
struct Linker_x09427077023280062;
...
# 1 "./exception/Er.h" 1
# 23 "./exception/Er.h"
struct Linker_x017383613095497052;
...
# 1 "./util/Bits.h" 1
# 21 "./util/Bits.h"
struct Linker_x09962483349852809;
...
struct Linker_x03007991942798458;
...
struct Linker_x08081595293824733;
...
struct Linker_x0443918459250741;
...
struct Linker_x015515107162857666;
...
struct Linker_x03448892414906406;
...
struct Linker_x04786572796670272;
...
struct Linker_x09141884751456124;
...
struct Linker_x0534203999289637;
...
struct Linker_x04143588870284247;
...
# 19 "./util/AddrTools.h"
struct Linker_x04412561297902129;
...
struct Linker_x09431794984253408;
...
# 21 "./util/ArrayList.h"
struct Linker_x07363267822037225;
...
# 107 "subnode/ReachabilityCollector.c"
    ...
    (pi)->Identity_verifier = ((unsigned long) 0x21fb8424b053c3fcull);
...
void ReachabilityCollector_change(struct ReachabilityCollector* rc, struct Address* nodeAddr)
{
    struct ReachabilityCollector_pvt* rcp = (__extension__ ({ __typeof__((struct ReachabilityCollector_pvt*) rc) Identity_ptr = ((struct ReachabilityCollector_pvt*) rc); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x21fb8424b053c3fcull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("ReachabilityCollector.c"), (142), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x21fb8424b053c3fcull))"); } } while (0); Identity_ptr; }));
...
static void onReplyTimeout(struct MsgCore_Promise* prom)
{
    ...
    struct ReachabilityCollector_pvt* rcp =
        (__extension__ ({ __typeof__((struct ReachabilityCollector_pvt*) q->rcp) Identity_ptr = ((struct ReachabilityCollector_pvt*) q->rcp); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x21fb8424b053c3fcull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("ReachabilityCollector.c"), (158), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x21fb8424b053c3fcull))"); } } while (0); Identity_ptr; }));
...
static void onReply(Dict* msg, struct Address* src, struct MsgCore_Promise* prom)
{
    struct Query* q = (struct Query*) prom->userData;
    struct ReachabilityCollector_pvt* rcp =
        (__extension__ ({ __typeof__((struct ReachabilityCollector_pvt*) q->rcp) Identity_ptr = ((struct ReachabilityCollector_pvt*) q->rcp); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x21fb8424b053c3fcull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("ReachabilityCollector.c"), (184), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x21fb8424b053c3fcull))"); } } while (0); Identity_ptr; }));
    if (!src) {
        onReplyTimeout(prom);
        mkNextRequest(rcp);
        return;
...
# 271 "subnode/ReachabilityCollector.c"
    ...
    if (!pi || !pi->pub.querying) {
        if (rcp->piList->length > 0) {
            do { if (rcp->log && Log_Level_DEBUG >= Log_Level_DEBUG) { Log_print(rcp->log, Log_Level_DEBUG, "ReachabilityCollector.c", 278, "All [%u] peers have been queried", rcp->piList->length); } } while (0);
        }
        return;
    }
    if (pi->waitForResponse) {
        do { if (rcp->log && Log_Level_DEBUG >= Log_Level_DEBUG) { Log_print(rcp->log, Log_Level_DEBUG, "ReachabilityCollector.c", 283, "Peer is waiting for response."); } } while (0);
        return;
    }
    queryPeer(rcp, pi);
}
...
{
    struct ReachabilityCollector_pvt* rcp = (__extension__ ({ __typeof__((struct ReachabilityCollector_pvt*) vrc) Identity_ptr = ((struct ReachabilityCollector_pvt*) vrc); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x21fb8424b053c3fcull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("ReachabilityCollector.c"), (291), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x21fb8424b053c3fcull))"); } } while (0); Identity_ptr; }));
    mkNextRequest(rcp);
    ...
    for (int j = 0; j < rcp->piList->length; j++) {
        ...
        do { if (rcp->log && Log_Level_DEBUG >= Log_Level_DEBUG) { Log_print(rcp->log, Log_Level_DEBUG, "ReachabilityCollector.c", 304, "Visiting peer [%" 
# 304 "subnode/ReachabilityCollector.c" 3
       "I64x" 
# 304 "subnode/ReachabilityCollector.c"
       "] samples [%u]", pi->pub.addr.path, pi->lagSamples); } } while (0)
                                              ;
        if (pi->lagSamples == 0) {
            do { if (rcp->log && Log_Level_DEBUG >= Log_Level_DEBUG) { Log_print(rcp->log, Log_Level_DEBUG, "ReachabilityCollector.c", 307, "Triggering a ping to peer [%" 
# 307 "subnode/ReachabilityCollector.c" 3
           "I64x" 
# 307 "subnode/ReachabilityCollector.c"
...
struct ReachabilityCollector_PeerInfo*
    ReachabilityCollector_getPeerInfo(struct ReachabilityCollector* rc, int peerNum)
{
    struct ReachabilityCollector_pvt* rcp = (__extension__ ({ __typeof__((struct ReachabilityCollector_pvt*) rc) Identity_ptr = ((struct ReachabilityCollector_pvt*) rc); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x21fb8424b053c3fcull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("ReachabilityCollector.c"), (341), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x21fb8424b053c3fcull))"); } } while (0); Identity_ptr; }));
    struct PeerInfo_pvt* pi = ArrayList_OfPeerInfo_pvt_get(rcp->piList, peerNum);
    return pi ? &pi->pub : 
# 343 "subnode/ReachabilityCollector.c" 3 4
                          ((void *)0)
# 343 "subnode/ReachabilityCollector.c"
                              ;
}
...
void ReachabilityCollector_lagSample(
    struct ReachabilityCollector* rc, uint64_t label, uint32_t milliseconds)
{
    struct ReachabilityCollector_pvt* rcp = (__extension__ ({ __typeof__((struct ReachabilityCollector_pvt*) rc) Identity_ptr = ((struct ReachabilityCollector_pvt*) rc); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x21fb8424b053c3fcull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("ReachabilityCollector.c"), (349), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x21fb8424b053c3fcull))"); } } while (0); Identity_ptr; }));
...
void ReachabilityCollector_updateBandwidthAndDrops(
    ...
    uint64_t sumOfKb)
{
    struct ReachabilityCollector_pvt* rcp = (__extension__ ({ __typeof__((struct ReachabilityCollector_pvt*) rc) Identity_ptr = ((struct ReachabilityCollector_pvt*) rc); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x21fb8424b053c3fcull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("ReachabilityCollector.c"), (362), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x21fb8424b053c3fcull))"); } } while (0); Identity_ptr; }));
    struct PeerInfo_pvt* pi = piForLabel(rcp, label);
    if (!pi) { return; }
...
static void dummyOnChange(
    ...
    struct ReachabilityCollector_PeerInfo* pi)
{
    struct ReachabilityCollector_pvt* rcp = (__extension__ ({ __typeof__((struct ReachabilityCollector_pvt*) rc) Identity_ptr = ((struct ReachabilityCollector_pvt*) rc); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x21fb8424b053c3fcull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("ReachabilityCollector.c"), (375), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x21fb8424b053c3fcull))"); } } while (0); Identity_ptr; }));
    do { if (rcp->log && Log_Level_DEBUG >= Log_Level_DEBUG) { Log_print(rcp->log, Log_Level_DEBUG, "ReachabilityCollector.c", 376, "dummyOnChange called, onChange unassigned"); } } while (0);
}
...
struct ReachabilityCollector* ReachabilityCollector_new(struct Allocator* allocator,
                                                        ...
                                                        struct EncodingScheme* myScheme)
{
    struct Allocator* alloc = Allocator__child((allocator),"ReachabilityCollector.c",387);
    struct ReachabilityCollector_pvt* rcp =
        Allocator__calloc((alloc),(sizeof(struct ReachabilityCollector_pvt)),(1),"ReachabilityCollector.c",389);
    rcp->myAddr = myAddr;
    rcp->msgCore = msgCore;
    rcp->alloc = alloc;
    ...
    (rcp)->Identity_verifier = ((unsigned long) 0x21fb8424b053c3fcull);
    Timeout__setInterval((cycle), (rcp), (2000), (base), (alloc), "ReachabilityCollector.c", 400);
    return &rcp->pub;
}
