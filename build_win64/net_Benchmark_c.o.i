...
struct Linker_x017179802983236847;
...
struct Linker_x02326099180955381;
# 33 "./util/Assert.h"
__attribute__((__format__ (__printf__, 1, 2)))
...
struct Linker_x08927150540341846;
...
struct Linker_x029599723502629494;
...
# 1 "./exception/Er.h" 1
# 23 "./exception/Er.h"
struct Linker_x008503671775876875;
...
struct Linker_x02900784634242364;
...
# 1 "./util/log/Log.h" 1
# 20 "./util/log/Log.h"
struct Linker_x046796572915693746;
...
struct Linker_x09363221256247407;
...
struct Linker_x04147941105430406;
...
# 1 "./util/CString.h" 1
# 21 "./util/CString.h"
struct Linker_x0916312998358866;
...
struct Linker_x06067616603171733;
...
struct Linker_x015948897191461553;
...
# 1 "./util/Bits.h" 1
# 21 "./util/Bits.h"
struct Linker_x017308505847996214;
...
struct Linker_x08648763844905445;
...
struct Linker_x045136025536834024;
...
struct Linker_x08807768488067953;
...
struct Linker_x02587706954844786;
...
struct Linker_x04820377728587262;
...
# 21 "./util/ArrayList.h"
struct Linker_x0629067261685083;
...
struct Linker_x06653303513420077;
...
struct Linker_x05314787063571385;
...
struct Linker_x02708079410227071;
...
# 1 "./dht/Address.h" 1
# 21 "./dht/Address.h"
struct Linker_x012248660347655838;
# 34 "./dht/Address.h"
struct Address
{
...
struct Linker_x06703050050483268;
...
struct Linker_x06533413040262892;
...
struct Linker_x08205107320867655;
...
struct Linker_x041368177857444266;
...
struct Linker_x07783843456347788;
...
struct Linker_x03888674918662014;
...
struct Linker_x04675033584812158;
...
# 1 "./benc/List.h" 1
# 21 "./benc/List.h"
struct Linker_x06418849761787571;
...
struct Linker_x0649516758570031;
...
struct Linker_x08419419044188385;
...
struct Linker_x01196022079847674;
...
struct Linker_x04604198706579421;
...
# 22 "./net/TUNAdapter.h"
struct Linker_x033200609799503344;
...
struct Linker_x02865573070904146;
...
{
    do { if (!((!ctx->benchName))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Benchmark.c"), (44), "(!ctx->benchName)"); } } while (0);
    ctx->benchName = benchName;
    ctx->itemName = itemName;
    ctx->items = items;
    do { if (ctx->log && Log_Level_INFO >= Log_Level_DEBUG) { Log_print(ctx->log, Log_Level_INFO, "Benchmark.c", 48, "----- Begin %s benchmark -----", benchName); } } while (0);
...
static void done(struct Context* ctx)
{
    ...
    do { if (ctx->log && Log_Level_INFO >= Log_Level_DEBUG) { Log_print(ctx->log, Log_Level_INFO, "Benchmark.c", 60, " "); } } while (0);
    do { if (ctx->log && Log_Level_INFO >= Log_Level_DEBUG) { Log_print(ctx->log, Log_Level_INFO, "Benchmark.c", 61, "---------------------------------------------------------------"); } } while (0);
    do { if (ctx->log && Log_Level_INFO >= Log_Level_DEBUG) { Log_print(ctx->log, Log_Level_INFO, "Benchmark.c", 62, "Benchmark %s in %dms. %d %s per second", ctx->benchName, (int)time, (int)kbps, ctx->itemName); } } while (0)
                                                            ;
    do { if (ctx->log && Log_Level_INFO >= Log_Level_DEBUG) { Log_print(ctx->log, Log_Level_INFO, "Benchmark.c", 64, "---------------------------------------------------------------"); } } while (0);
    do { if (ctx->log && Log_Level_INFO >= Log_Level_DEBUG) { Log_print(ctx->log, Log_Level_INFO, "Benchmark.c", 65, " "); } } while (0);
    do { if (!((ctx->benchName))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Benchmark.c"), (66), "(ctx->benchName)"); } } while (0);
    ctx->benchName = 
# 67 "net/Benchmark.c" 3 4
                    ((void *)0)
# 67 "net/Benchmark.c"
                        ;
}
...
{
    do { if (ctx->log && Log_Level_INFO >= Log_Level_DEBUG) { Log_print(ctx->log, Log_Level_INFO, "Benchmark.c", 72, "Setting up salsa20/poly1305 benchmark (encryption and decryption only)"); } } while (0);
    struct Allocator* alloc = Allocator__child((ctx->alloc),"Benchmark.c",73);
    struct CryptoAuth* ca1 = CryptoAuth_new(alloc, 
# 74 "net/Benchmark.c" 3 4
                                                  ((void *)0)
# 74 "net/Benchmark.c"
...
# 120 "net/Benchmark.c"
       )); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x2ded615d0836b0d2ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Benchmark.c"), (120), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x2ded615d0836b0d2ull))"); } } while (0); Identity_ptr; })); }));
...
# 127 "net/Benchmark.c"
       )); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x2ded615d0836b0d2ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Benchmark.c"), (127), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x2ded615d0836b0d2ull))"); } } while (0); Identity_ptr; })); }));
...
# 134 "net/Benchmark.c"
       )); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x2ded615d0836b0d2ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Benchmark.c"), (134), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x2ded615d0836b0d2ull))"); } } while (0); Identity_ptr; })); }));

    sc->msgCount++;
    return 
# 137 "net/Benchmark.c" 3 4
          ((void *)0)
# 137 "net/Benchmark.c"
              ;
}
...
{
    do { if (ctx->log && Log_Level_INFO >= Log_Level_DEBUG) { Log_print(ctx->log, Log_Level_INFO, "Benchmark.c", 142, "Setting up salsa20/poly1305 benchmark (encryption and decryption only)"); } } while (0);
    struct Allocator* alloc = Allocator__child((ctx->alloc),"Benchmark.c",143);;
    uint8_t ipv6[16];
    uint8_t publicKey[32];
    ...
    struct SwitchingContext* sc = Allocator__calloc((alloc),(sizeof(struct SwitchingContext)),(1),"Benchmark.c",151);
    (sc)->Identity_verifier = ((unsigned long) 0x2ded615d0836b0d2ull);
    sc->benchmarkCtx = ctx;
...
# 176 "net/Benchmark.c" 3 4
                                               ((void *)0)
# 176 "net/Benchmark.c"
                                                   ,
                                                
# 177 "net/Benchmark.c" 3 4
                                               ((void *)0)
# 177 "net/Benchmark.c"
                                                   ,
                                                alloc);
    do { if (!((!ret))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Benchmark.c"), (179), "(!ret)"); } } while (0);
    ...
    do {             struct Er_Ret* Er_ret = Message_epush(msg,    ((void *)0)   , (56 + 12));                          if (Er_ret) {                 struct Er_Ret** Er_errOut = (struct Er_Ret**)0;                 if (Er_errOut) {                     *Er_errOut = Er_unwind("Benchmark.c", 189, Er_ret);                 } else {                     Er__assertFail(Er_unwind("Benchmark.c", 189, Er_ret));                 }             }                  } while (0);
    ...
    for (int i = 1; i < count; i++) {
        ...
        do { if (!((msg->bytes == (void*)rh))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Benchmark.c"), (208), "(msg->bytes == (void*)rh)"); } } while (0);
        do { if (!((sc->msgCount == i))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Benchmark.c"), (209), "(sc->msgCount == i)"); } } while (0);
        do { if (!((ping->magic == ((uint32_t)__builtin_bswap32(((uint32_t)(0x9d74e35b))))))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Benchmark.c"), (210), "(ping->magic == ((uint32_t)__builtin_bswap32(((uint32_t)(0x9d74e35b)))))"); } } while (0);
        do { if (!((ch->type_be = ((uint16_t)(__extension__ ({ uint16_t _x = (((uint16_t)(4))); _x >> 8 | _x << 8; })))))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Benchmark.c"), (211), "(ch->type_be = ((uint16_t)(__extension__ ({ uint16_t _x = (((uint16_t)(4))); _x >> 8 | _x << 8; }))))"); } } while (0);
        do { if (!((!Checksum_engine((void*)ch, 8 + 4)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Benchmark.c"), (212), "(!Checksum_engine((void*)ch, 8 + 4))"); } } while (0);
    }
    done(ctx);

    do { if (ctx->log && Log_Level_INFO >= Log_Level_DEBUG) { Log_print(ctx->log, Log_Level_INFO, "Benchmark.c", 216, "DONE"); } } while (0);
    Allocator__free((alloc),"Benchmark.c",217);
}


void Benchmark_runAll(void)
{
    struct Allocator* alloc = MallocAllocator__new((1<<22),"Benchmark.c",223);
    struct Context* ctx = Allocator__calloc((alloc),(sizeof(struct Context)),(1),"Benchmark.c",224);
    (ctx)->Identity_verifier = ((unsigned long) 0x2ded615d0836b0d2ull);
    ctx->alloc = alloc;
    ctx->base = EventBase_new(alloc);
    struct Log* log = ctx->log = FileWriterLog_new(
# 228 "net/Benchmark.c" 3
                                                  (__acrt_iob_func(1))
# 228 "net/Benchmark.c"
                                                        , alloc);
    ctx->rand = Random_new(alloc, log, 
# 229 "net/Benchmark.c" 3 4
                                      ((void *)0)
# 229 "net/Benchmark.c"
                                          );
