...
# 1 "./util/Linker.h" 1
# 20 "./util/Assert.h" 2
struct Linker_x007839107969853543;
# 33 "./util/Assert.h"
__attribute__((__format__ (__printf__, 1, 2)))
...
struct Linker_x021124050363217473;
...
struct Linker_x07264076084675153;
...
# 1 "./util/Bits.h" 1
# 21 "./util/Bits.h"
struct Linker_x015990672963224384;
...
struct Linker_x008691023520417063;
...
struct Linker_x09580938999911506;
...
struct Linker_x08108730377612399;
...
struct Linker_x08919458462202683;
...
# 1 "./util/log/Log.h" 1
# 20 "./util/log/Log.h"
struct Linker_x0003468519483917021;
...
struct Linker_x0861827999532411;
...
struct Linker_x0651386758008206;
...
# 1 "./util/CString.h" 1
# 21 "./util/CString.h"
struct Linker_x008097890432914312;
...
struct Linker_x08080050204342892;
...
# 1 "./dht/Address.h" 1
# 21 "./dht/Address.h"
struct Linker_x09139446830519649;
# 34 "./dht/Address.h"
struct Address
{
...
struct Linker_x029860100943245316;
...
struct Linker_x05989159763811482;
...
struct Linker_x08659390874961612;
...
# 1 "./benc/List.h" 1
# 21 "./benc/List.h"
struct Linker_x07000997711124501;
...
struct Linker_x06562521081589174;
...
struct Linker_x06797539864635054;
...
struct Linker_x03526833752907266;
...
struct Linker_x08539409350077312;
...
struct Linker_x08115420851466182;
...
struct Linker_x07305556938702735;
...
struct Linker_x004456679452953449;
...
struct Linker_x09327395417380147;
...
# 19 "./util/AddrTools.h"
struct Linker_x0322370284818275;
...
{
    struct Context* ctx = (__extension__ ({ __typeof__((struct Context*) vctx) Identity_ptr = ((struct Context*) vctx); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x56b6ede0946c9af4ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("SearchRunner_admin.c"), (35), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x56b6ede0946c9af4ull))"); } } while (0); Identity_ptr; }));
...
static void searchResponse(struct RouterModule_Promise* promise,
                           ...
                           Dict* responseDict)
{
    struct Search* search = (__extension__ ({ __typeof__((struct Search*) promise->userData) Identity_ptr = ((struct Search*) promise->userData); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x56b6ede0946c9af4ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("SearchRunner_admin.c"), (79), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x56b6ede0946c9af4ull))"); } } while (0); Identity_ptr; }));
...
{
    struct Context* ctx = (__extension__ ({ __typeof__((struct Context*) vctx) Identity_ptr = ((struct Context*) vctx); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x56b6ede0946c9af4ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("SearchRunner_admin.c"), (109), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x56b6ede0946c9af4ull))"); } } while (0); Identity_ptr; }));
...
# 126 "dht/dhtcore/SearchRunner_admin.c"
        ...
        s->alloc = alloc;
        (s)->Identity_verifier = ((unsigned long) 0x56b6ede0946c9af4ull);
                                 ...
                                 struct Allocator* alloc)
{
    struct Context* ctx = Allocator__clone((alloc),((&(struct Context) { .admin = admin, .allocator = alloc, .runner = runner })),sizeof(*((&(struct Context) { .admin = admin, .allocator = alloc, .runner = runner }))),"SearchRunner_admin.c",148)
       ...
       ;
    (ctx)->Identity_verifier = ((unsigned long) 0x56b6ede0946c9af4ull);
