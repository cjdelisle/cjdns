...
# 1 "./util/Linker.h" 1
# 20 "./util/Assert.h" 2
struct Linker_x06383135868190692;
# 33 "./util/Assert.h"
__attribute__((__format__ (__printf__, 1, 2)))
...
struct Linker_x027988644841823107;
...
# 1 "./util/CString.h" 1
# 21 "./util/CString.h"
struct Linker_x03866638836646601;
...
struct Linker_x05697893855097664;
...
# 19 "./util/log/FileWriterLog.h"
# 1 "./util/log/Log.h" 1
# 20 "./util/log/Log.h"
struct Linker_x04027599883998034;
...
struct Linker_x05004875116181053;
...
# 17 "./util/test/Seccomp_test.c" 2

# 1 "./memory/MallocAllocator.h" 1
# 21 "./memory/MallocAllocator.h"
struct Linker_x0581678097479718;
...
# 1 "./exception/Er.h" 1
# 23 "./exception/Er.h"
struct Linker_x0666360889835377;
...
struct Linker_x03259710942053211;
...
struct Linker_x009200553312941029;
...
# 1 "./util/events/PipeServer.h" 1
# 19 "./util/events/PipeServer.h"
# 1 "./exception/Except.h" 1
# 21 "./exception/Except.h"
struct Linker_x08269351805789076;
...
# 1 "./util/Bits.h" 1
# 21 "./util/Bits.h"
struct Linker_x0986949235042305;
...
struct Linker_x017790984945840216;
...
struct Linker_x008906650062882782;
...
# 21 "./util/events/PipeServer.h" 2


struct Linker_x004205228733201971;
...
# 24 "./util/events/Pipe.h"
struct Linker_x01181614659137844;
...
# 63 "./util/events/Pipe.h"
__attribute__ ((warn_unused_result)) struct Er_Ret* Pipe_forFd(struct Pipe* *Er_returnValP, int fd, _Bool ipc, struct EventBase* eb, struct Log* log, struct Allocator* userAlloc)



                                                     ;

__attribute__ ((warn_unused_result)) struct Er_Ret* Pipe_named(struct Pipe* *Er_returnValP, const char* fullPath, struct EventBase* eb, struct Log* log, struct Allocator* userAlloc)


                                                     ;

__attribute__ ((warn_unused_result)) struct Er_Ret* Pipe_exists(_Bool *Er_returnValP, const char* path, struct Allocator* errAlloc);
# 24 "./util/test/Seccomp_test.c" 2
# 1 "./util/events/Timeout.h" 1
# 20 "./util/events/Timeout.h"
struct Linker_x0388167790798869;
...
struct Linker_x05080884383898343;
...
struct Linker_x0785200158325694;
...
# 29 "./util/test/Seccomp_test.c" 2


# 30 "./util/test/Seccomp_test.c"
struct Context
{
    struct Iface iface;
    struct Allocator* alloc;
...
{
    struct ChildCtx* child = (__extension__ ({ __typeof__((struct ChildCtx*) pipe->userData) Identity_ptr = ((struct ChildCtx*) pipe->userData); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0xd72d1465577daa70ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Seccomp_test.c"), (54), "(Identity_ptr->Identity_verifier == ((unsigned long) 0xd72d1465577daa70ull))"); } } while (0); Identity_ptr; }));

    do {             struct Er_Ret* Er_ret = Seccomp_dropPermissions(child->alloc, child->log);                          if (Er_ret) {                 struct Er_Ret** Er_errOut = (struct Er_Ret**)0;                 if (Er_errOut) {                     *Er_errOut = Er_unwind("Seccomp_test.c", 56, Er_ret);                 } else {                     Er__assertFail(Er_unwind("Seccomp_test.c", 56, Er_ret));                 }             }                  } while (0);
    do { if (!((Seccomp_isWorking()))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Seccomp_test.c"), (57), "(Seccomp_isWorking())"); } } while (0);

    struct Message* ok = Message_new(0, 512, child->alloc);
    do {             struct Er_Ret* Er_ret = Message_epush(ok, "OK", 3);                          if (Er_ret) {                 struct Er_Ret** Er_errOut = (struct Er_Ret**)0;                 if (Er_errOut) {                     *Er_errOut = Er_unwind("Seccomp_test.c", 60, Er_ret);                 } else {                     Er__assertFail(Er_unwind("Seccomp_test.c", 60, Er_ret));                 }             }                  } while (0);

    struct Iface iface = { .send = 
# 62 "./util/test/Seccomp_test.c" 3 4
                                  ((void *)0) 
# 62 "./util/test/Seccomp_test.c"
                                       };
    ...
    Timeout__setInterval((childComplete), (child->base), (10), (child->base), (child->alloc), "Seccomp_test.c", 68);
}

static void timeout(void* vNULL)
{
    do { if (!((!"timed out"))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Seccomp_test.c"), (73), "(!\"timed out\")"); } } while (0);
}

static void timeout2(void* vNULL)
{
    do { if (!((!"time out 2"))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Seccomp_test.c"), (78), "(!\"time out 2\")"); } } while (0);
}

static int child(char* pipeName, struct Allocator* alloc, struct Log* logger)
{
    struct ChildCtx* ctx = Allocator__calloc((alloc),(sizeof(struct ChildCtx)),(1),"Seccomp_test.c",83);
    ctx->base = EventBase_new(alloc);
    ctx->alloc = alloc;
    ctx->log = logger;
    ctx->pipe = (__extension__({             struct Pipe* Er_returnVal;             __builtin_memset(&Er_returnVal, 0, sizeof(Er_returnVal));             struct Er_Ret* Er_ret = Pipe_named(&Er_returnVal, pipeName, ctx->base, logger, alloc);                          if (Er_ret) {                 struct Er_Ret** Er_errOut = (struct Er_Ret**)0;                 if (Er_errOut) {                     *Er_errOut = Er_unwind("Seccomp_test.c", 87, Er_ret);                 } else {                     Er__assertFail(Er_unwind("Seccomp_test.c", 87, Er_ret));                 }             }                      Er_returnVal;         }));
    ctx->pipe->onConnection = onConnectionChild;
    ctx->pipe->userData = ctx;
    (ctx)->Identity_verifier = ((unsigned long) 0xd72d1465577daa70ull);
    Timeout__setTimeout((timeout), (ctx->base), (2000), (ctx->base), (alloc), "Seccomp_test.c", 91);
...
{
    struct Context* ctx = (__extension__ ({ __typeof__((struct Context*) iface) Identity_ptr = ((struct Context*) iface); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0xd72d1465577daa70ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Seccomp_test.c"), (99), "(Identity_ptr->Identity_verifier == ((unsigned long) 0xd72d1465577daa70ull))"); } } while (0); Identity_ptr; }));

    (__extension__({             struct Sockaddr* Er_returnVal;             __builtin_memset(&Er_returnVal, 0, sizeof(Er_returnVal));             struct Er_Ret* Er_ret = AddrIface_popAddr(&Er_returnVal, msg);                          if (Er_ret) {                 struct Er_Ret** Er_errOut = (struct Er_Ret**)0;                 if (Er_errOut) {                     *Er_errOut = Er_unwind("Seccomp_test.c", 101, Er_ret);                 } else {                     Er__assertFail(Er_unwind("Seccomp_test.c", 101, Er_ret));                 }             }                      Er_returnVal;         }));
    do { if (!((msg->length == 3))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Seccomp_test.c"), (102), "(msg->length == 3)"); } } while (0);
    do { if (!((!__builtin_memcmp(msg->bytes,"OK",3)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Seccomp_test.c"), (103), "(!__builtin_memcmp(msg->bytes,\"OK\",3))"); } } while (0);
...
{
    struct Allocator* alloc = MallocAllocator__new((20000),"Seccomp_test.c",110);
    struct Log* logger = FileWriterLog_new(
# 111 "./util/test/Seccomp_test.c" 3
                                          (__acrt_iob_func(1))
# 111 "./util/test/Seccomp_test.c"
                                                , alloc);

    if (!Seccomp_exists()) {
        do { if (logger && Log_Level_DEBUG >= Log_Level_DEBUG) { Log_print(logger, Log_Level_DEBUG, "Seccomp_test.c", 114, "Seccomp not supported on this system"); } } while (0);
        return 0;
    }
    if (argc > 3 && !CString_strcmp("Seccomp_test", argv[1]) && !CString_strcmp("child", argv[2])) {
        child(argv[3], alloc, logger);
        Allocator__free((alloc),"Seccomp_test.c",119);
        return 0;
    }

    struct EventBase* eb = EventBase_new(alloc);
    struct Random* rand = Random_new(alloc, logger, 
# 124 "./util/test/Seccomp_test.c" 3 4
                                                   ((void *)0)
# 124 "./util/test/Seccomp_test.c"
                                                       );
    char randName[32] = {0};
    Random_base32(rand, (uint8_t*)randName, 31);
    String* name = String_printf(alloc, "%s%scjdns-test-%s", "/tmp", "/", randName);

    struct Context* ctx = Allocator__calloc((alloc),(sizeof(struct Context)),(1),"Seccomp_test.c",129);
    (ctx)->Identity_verifier = ((unsigned long) 0xd72d1465577daa70ull);
    ctx->alloc = alloc;
    ctx->iface.send = receiveMessageParent;
    ctx->eventBase = eb;

    struct PipeServer* pipe = PipeServer_named(name->bytes, eb, 
# 135 "./util/test/Seccomp_test.c" 3 4
                                                               ((void *)0)
# 135 "./util/test/Seccomp_test.c"
                                                                   , logger, alloc);
    Iface_plumb(&ctx->iface, &pipe->iface.iface);

    char* path = Process_getPath(alloc);
    char* args[] = { "Seccomp_test", "child", name->bytes, 
# 139 "./util/test/Seccomp_test.c" 3 4
                                                          ((void *)0) 
# 139 "./util/test/Seccomp_test.c"
                                                               };

    do { if (!((!Process_spawn(path, args, eb, alloc, 
# 141 "./util/test/Seccomp_test.c" 3 4
   ((void *)0)
# 141 "./util/test/Seccomp_test.c"
   )))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Seccomp_test.c"), (141), "(!Process_spawn(path, args, eb, alloc, ((void *)0)))"); } } while (0);

    Timeout__setTimeout((timeout2), (
# 143 "./util/test/Seccomp_test.c" 3 4
   ((void *)0)
# 143 "./util/test/Seccomp_test.c"
   ), (2000), (eb), (alloc), "Seccomp_test.c", 143);
