...
# 1 "./util/Linker.h" 1
# 20 "./util/Assert.h" 2
struct Linker_x07048574423138263;
# 33 "./util/Assert.h"
__attribute__((__format__ (__printf__, 1, 2)))
...
struct Linker_x05171832814801554;
...
{
    struct Allocator* alloc = MallocAllocator__new((2048),"Allocator_test.c",48);
    int calls = 0;
    struct TestStruct* ts = Allocator__clone((alloc),((&(struct TestStruct) { .value = increment(&calls) })),sizeof(*((&(struct TestStruct) { .value = increment(&calls) }))),"Allocator_test.c",50)

       ;
    do { if (!((calls == 1))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Allocator_test.c"), (53), "(calls == 1)"); } } while (0);
    do { if (!((ts->value == 1))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Allocator_test.c"), (54), "(ts->value == 1)"); } } while (0);
    Allocator__free((alloc),"Allocator_test.c",55);
}

static void structureSizes()
{
    struct Allocator* alloc = MallocAllocator__new((2048),"Allocator_test.c",60);
    size_t bytesUsed;

    bytesUsed = Allocator_bytesAllocated(alloc);
    do { if (!((bytesUsed == sizeof(struct Allocator_Allocation_pvt) + sizeof(long) + sizeof(struct Allocator_FirstCtx)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Allocator_test.c"), (64), "(bytesUsed == sizeof(struct Allocator_Allocation_pvt) + sizeof(long) + sizeof(struct Allocator_FirstCtx))"); } } while (0);
    Allocator__malloc((alloc),(25),"Allocator_test.c",65);
    bytesUsed += (((25 / sizeof(char*)) + 1) * sizeof(char*)) + sizeof(struct Allocator_Allocation_pvt) + sizeof(long);
    do { if (!((Allocator_bytesAllocated(alloc) == bytesUsed))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Allocator_test.c"), (67), "(Allocator_bytesAllocated(alloc) == bytesUsed)"); } } while (0);

    struct Allocator* child = Allocator__child((alloc),"Allocator_test.c",69);
    bytesUsed += sizeof(struct Allocator_Allocation_pvt) + sizeof(long) + sizeof(struct Allocator_pvt);
    do { if (!((Allocator_bytesAllocated(alloc) == bytesUsed))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Allocator_test.c"), (71), "(Allocator_bytesAllocated(alloc) == bytesUsed)"); } } while (0);

    Allocator__malloc((child),(30),"Allocator_test.c",73);
    bytesUsed += 32 + sizeof(struct Allocator_Allocation_pvt) + sizeof(long);
    do { if (!((Allocator_bytesAllocated(alloc) == bytesUsed))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Allocator_test.c"), (75), "(Allocator_bytesAllocated(alloc) == bytesUsed)"); } } while (0);

    Allocator__free((child),"Allocator_test.c",77);
    bytesUsed -= 32 + sizeof(struct Allocator_Allocation_pvt) + sizeof(long);
    bytesUsed -= sizeof(struct Allocator_Allocation_pvt) + sizeof(long) + sizeof(struct Allocator_pvt);
    do { if (!((Allocator_bytesAllocated(alloc) == bytesUsed))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Allocator_test.c"), (80), "(Allocator_bytesAllocated(alloc) == bytesUsed)"); } } while (0);

    Allocator__free((alloc),"Allocator_test.c",82);
}
