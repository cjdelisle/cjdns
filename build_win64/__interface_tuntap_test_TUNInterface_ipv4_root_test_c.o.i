...
# 1 "./util/Linker.h" 1
# 20 "./util/Assert.h" 2
struct Linker_x02656774499960528;
# 33 "./util/Assert.h"
__attribute__((__format__ (__printf__, 1, 2)))
...
struct Linker_x07586881460419928;
...
struct Linker_x0426098056916534;
...
# 21 "./util/Bits.h"
struct Linker_x040399106410854224;
...
struct Linker_x0816819532964596;
...
struct Linker_x08110745378519157;
...
# 1 "./util/log/Log.h" 1
# 20 "./util/log/Log.h"
struct Linker_x025292435165150917;
...
struct Linker_x05202194337030419;
...
struct Linker_x04265025240530662;
...
struct Linker_x07223743994224456;
...
struct Linker_x05740656874491303;
...
struct Linker_x09413301368671187;
...
struct Linker_x09209213826485374;
...
struct Linker_x09892693784628293;
...
# 1 "./util/CString.h" 1
# 21 "./util/CString.h"
struct Linker_x08989565160833213;
...
struct Linker_x06794550709384941;
...
static __attribute__ ((warn_unused_result)) struct Iface* receiveMessageTUN(struct Message* msg, struct TUNTools* tt)
{
    ...
    do {             struct Er_Ret* Er_ret = TUNMessageType_push(msg, ethertype);                          if (Er_ret) {                 struct Er_Ret** Er_errOut = (struct Er_Ret**)0;                 if (Er_errOut) {                     *Er_errOut = Er_unwind("TUNInterface_ipv4_root_test.c", 52, Er_ret);                 } else {                     Er__assertFail(Er_unwind("TUNInterface_ipv4_root_test.c", 52, Er_ret));                 }             }                  } while (0);
...
# 66 "./interface/tuntap/test/TUNInterface_ipv4_root_test.c"
    ...
    struct Iface* tun = (__extension__({             struct Iface* Er_returnVal;             __builtin_memset(&Er_returnVal, 0, sizeof(Er_returnVal));             struct Er_Ret* Er_ret = TUNInterface_new(&Er_returnVal,                        ((void *)0)                       , assignedIfName, 0, base, logger, alloc);                          if (Er_ret) {                 struct Er_Ret** Er_errOut = (struct Er_Ret**)0;                 if (Er_errOut) {                     *Er_errOut = Er_unwind("TUNInterface_ipv4_root_test.c", 72, Er_ret);                 } else {                     Er__assertFail(Er_unwind("TUNInterface_ipv4_root_test.c", 72, Er_ret));                 }             }                      Er_returnVal;         }));
    addrA->flags |= (1<<1);
    addrA->prefix = 30;
    do {             struct Er_Ret* Er_ret = NetDev_addAddress(assignedIfName, addrA, logger, alloc);                          if (Er_ret) {                 struct Er_Ret** Er_errOut = (struct Er_Ret**)0;                 if (Er_errOut) {                     *Er_errOut = Er_unwind("TUNInterface_ipv4_root_test.c", 75, Er_ret);                 } else {                     Er__assertFail(Er_unwind("TUNInterface_ipv4_root_test.c", 75, Er_ret));                 }             }                  } while (0);
