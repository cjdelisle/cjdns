# 1 "interface/addressable/AddrIfaceMuxer.c"
# 1 "/home/jojapoppa/cjdns//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "interface/addressable/AddrIfaceMuxer.c"
# 15 "interface/addressable/AddrIfaceMuxer.c"
# 1 "./interface/addressable/AddrIfaceMuxer.h" 1
# 18 "./interface/addressable/AddrIfaceMuxer.h"
...
# 1 "./util/Linker.h" 1
# 20 "./util/Assert.h" 2
struct Linker_x08912084186998472;
# 33 "./util/Assert.h"
__attribute__((__format__ (__printf__, 1, 2)))
...
struct Linker_x038569421309122665;
...
struct Linker_x04872392411692059;
...
# 1 "./util/Bits.h" 1
# 21 "./util/Bits.h"
struct Linker_x08608305747788529;
...
struct Linker_x014223301036708902;
...
struct Linker_x014832241909323085;
...
struct Linker_x09688787426730328;
...
# 19 "./interface/addressable/AddrIfaceMuxer.h" 2

# 1 "./util/log/Log.h" 1
# 20 "./util/log/Log.h"
struct Linker_x08655623143039299;
...
void Log_print(struct Log* log,
               ...
               const char* format,
               ...);
# 21 "./interface/addressable/AddrIfaceMuxer.h" 2

struct Linker_x032636136639538016;
...
# 16 "interface/addressable/AddrIfaceMuxer.c" 2
...
# 1 "./util/Hex.h" 1
# 21 "./util/Hex.h"
struct Linker_x08707513097147503;
...
# 23 "interface/addressable/AddrIfaceMuxer.c" 2
...
struct Map_Ifaces
{
    ...
    struct AddrIfaceMuxer_Iface** values;
...
static inline struct Map_Ifaces* Map_Ifaces_new(struct Allocator* allocator)
{
    return Allocator__clone((allocator),((&(struct Map_Ifaces) { .allocator = allocator })),sizeof(*((&(struct Map_Ifaces) { .allocator = allocator }))),"Map.h",85)

       ;
}
# 110 "./util/Map.h"
static inline int Map_Ifaces_indexForHandle(uint32_t handle, struct Map_Ifaces* map)
{
    uint32_t base = 0;
...
static inline int Map_Ifaces_remove(int index, struct Map_Ifaces* map)
{
...
# 146 "./util/Map.h"
            ...
            __builtin_memmove(&map->values[index],&map->values[index + 1],(map->count - index - 1) * sizeof(struct AddrIfaceMuxer_Iface*))
...
static inline int Map_Ifaces_put(struct AddrIfaceMuxer_Iface** value,
                                    struct Map_Ifaces* map)
...
# 190 "./util/Map.h"
        ...
        map->values = Allocator__realloc((map->allocator),(map->values),(sizeof(struct AddrIfaceMuxer_Iface*) * (map->count + 10)),"Map.h",195)
    ...
    Bits__memcpy(&map->values[i],value,sizeof(struct AddrIfaceMuxer_Iface*),"Map.h",220);

    return i;
}
# 37 "interface/addressable/AddrIfaceMuxer.c" 2
...
static __attribute__ ((warn_unused_result)) struct Iface* incomingFromAddrIf(struct Message* msg, struct Iface* addrIf)
{
    struct AddrIfaceMuxer_pvt* ctx =
        (__extension__ ({ const __typeof__(((struct AddrIfaceMuxer_pvt*)0)->pub.iface.iface)*__mptr = (addrIf); (__extension__ ({ __typeof__((struct AddrIfaceMuxer_pvt*)((char*)__mptr - 
# 49 "interface/addressable/AddrIfaceMuxer.c" 3 4
       __builtin_offsetof (
# 49 "interface/addressable/AddrIfaceMuxer.c"
       struct AddrIfaceMuxer_pvt
# 49 "interface/addressable/AddrIfaceMuxer.c" 3 4
       , 
# 49 "interface/addressable/AddrIfaceMuxer.c"
...
# 49 "interface/addressable/AddrIfaceMuxer.c"
       )) Identity_ptr = ((struct AddrIfaceMuxer_pvt*)((char*)__mptr - 
# 49 "interface/addressable/AddrIfaceMuxer.c" 3 4
       __builtin_offsetof (
# 49 "interface/addressable/AddrIfaceMuxer.c"
       struct AddrIfaceMuxer_pvt
...
# 49 "interface/addressable/AddrIfaceMuxer.c"
       )); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x6a2785b50a142902ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("AddrIfaceMuxer.c"), (49), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x6a2785b50a142902ull))"); } } while (0); Identity_ptr; })); }));

    struct Sockaddr* addr = (__extension__({             struct Sockaddr* Er_returnVal;             __builtin_memset(&Er_returnVal, 0, sizeof(Er_returnVal));             struct Er_Ret* Er_ret = AddrIface_popAddr(&Er_returnVal, msg);                          if (Er_ret) {                 struct Er_Ret** Er_errOut = (struct Er_Ret**)0;                 if (Er_errOut) {                     *Er_errOut = Er_unwind("AddrIfaceMuxer.c", 51, Er_ret);                 } else {                     Er__assertFail(Er_unwind("AddrIfaceMuxer.c", 51, Er_ret));                 }             }                      Er_returnVal;         }));
    if (addr->addrLen > sizeof(struct Sockaddr)) {

        struct Sockaddr* subaddr = &addr[1];
        do { if (!((subaddr->addrLen == addr->addrLen - sizeof(struct Sockaddr)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("AddrIfaceMuxer.c"), (55), "(subaddr->addrLen == addr->addrLen - sizeof(struct Sockaddr))"); } } while (0);

        do {             struct Er_Ret* Er_ret = Message_eshift(msg, subaddr->addrLen);                          if (Er_ret) {                 struct Er_Ret** Er_errOut = (struct Er_Ret**)0;                 if (Er_errOut) {                     *Er_errOut = Er_unwind("AddrIfaceMuxer.c", 57, Er_ret);                 } else {                     Er__assertFail(Er_unwind("AddrIfaceMuxer.c", 57, Er_ret));                 }             }                  } while (0);
        addr->addrLen -= subaddr->addrLen;
    }
    uint32_t handle = Sockaddr_addrHandle(addr);
    int idx = Map_Ifaces_indexForHandle(handle, &ctx->ifaces);
    if (idx < 0) {
        do { if (ctx->log && Log_Level_INFO >= Log_Level_DEBUG) { Log_print(ctx->log, Log_Level_INFO, "AddrIfaceMuxer.c", 63, "DROP message to nonexistant iface [0x%x]", handle); } } while (0);
        return 
# 64 "interface/addressable/AddrIfaceMuxer.c" 3 4
              ((void *)0)
# 64 "interface/addressable/AddrIfaceMuxer.c"
                  ;
    }
...
static __attribute__ ((warn_unused_result)) struct Iface* incomingFromInputIf(struct Message* msg, struct Iface* inputIf)
{
    struct AddrIfaceMuxer_Iface* cli =
        (__extension__ ({ const __typeof__(((struct AddrIfaceMuxer_Iface*)0)->iface)*__mptr = (inputIf); (__extension__ ({ __typeof__((struct AddrIfaceMuxer_Iface*)((char*)__mptr - 
# 72 "interface/addressable/AddrIfaceMuxer.c" 3 4
       __builtin_offsetof (
# 72 "interface/addressable/AddrIfaceMuxer.c"
       struct AddrIfaceMuxer_Iface
# 72 "interface/addressable/AddrIfaceMuxer.c" 3 4
       , 
# 72 "interface/addressable/AddrIfaceMuxer.c"
...
# 72 "interface/addressable/AddrIfaceMuxer.c"
       )) Identity_ptr = ((struct AddrIfaceMuxer_Iface*)((char*)__mptr - 
# 72 "interface/addressable/AddrIfaceMuxer.c" 3 4
       __builtin_offsetof (
# 72 "interface/addressable/AddrIfaceMuxer.c"
       struct AddrIfaceMuxer_Iface
...
# 72 "interface/addressable/AddrIfaceMuxer.c"
       )); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x6a2785b50a142902ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("AddrIfaceMuxer.c"), (72), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x6a2785b50a142902ull))"); } } while (0); Identity_ptr; })); }));
    struct AddrIfaceMuxer_pvt* ctx = (__extension__ ({ __typeof__(cli->muxer) Identity_ptr = (cli->muxer); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x6a2785b50a142902ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("AddrIfaceMuxer.c"), (73), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x6a2785b50a142902ull))"); } } while (0); Identity_ptr; }));
    if (msg->length < (int)sizeof(struct Sockaddr)) {
        do { if (ctx->log && Log_Level_INFO >= Log_Level_DEBUG) { Log_print(ctx->log, Log_Level_INFO, "AddrIfaceMuxer.c", 75, "DROP runt"); } } while (0);
        return 
# 76 "interface/addressable/AddrIfaceMuxer.c" 3 4
              ((void *)0)
# 76 "interface/addressable/AddrIfaceMuxer.c"
                  ;
    }

    uint16_t addrLen = Bits_get16(msg->bytes);
    do {             struct Er_Ret* Er_ret = AddrIface_pushAddr(msg, &cli->addr);                          if (Er_ret) {                 struct Er_Ret** Er_errOut = (struct Er_Ret**)0;                 if (Er_errOut) {                     *Er_errOut = Er_unwind("AddrIfaceMuxer.c", 80, Er_ret);                 } else {                     Er__assertFail(Er_unwind("AddrIfaceMuxer.c", 80, Er_ret));                 }             }                  } while (0);
...
{
    struct AddrIfaceMuxer_Iface* cli = (__extension__ ({ __typeof__((struct AddrIfaceMuxer_Iface*)job->userData) Identity_ptr = ((struct AddrIfaceMuxer_Iface*)job->userData); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x6a2785b50a142902ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("AddrIfaceMuxer.c"), (90), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x6a2785b50a142902ull))"); } } while (0); Identity_ptr; }));
    struct AddrIfaceMuxer_pvt* m = (__extension__ ({ __typeof__(cli->muxer) Identity_ptr = (cli->muxer); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x6a2785b50a142902ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("AddrIfaceMuxer.c"), (91), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x6a2785b50a142902ull))"); } } while (0); Identity_ptr; }));
...
struct Iface* AddrIfaceMuxer_registerIface(struct AddrIfaceMuxer* muxer, struct Allocator* alloc)
{
    struct AddrIfaceMuxer_pvt* m = (__extension__ ({ __typeof__((struct AddrIfaceMuxer_pvt*) muxer) Identity_ptr = ((struct AddrIfaceMuxer_pvt*) muxer); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x6a2785b50a142902ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("AddrIfaceMuxer.c"), (102), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x6a2785b50a142902ull))"); } } while (0); Identity_ptr; }));
    struct AddrIfaceMuxer_Iface* cli =
        Allocator__calloc((alloc),(sizeof(struct AddrIfaceMuxer_Iface)),(1),"AddrIfaceMuxer.c",104);
    cli->iface.send = incomingFromInputIf;
    cli->muxer = m;
    cli->alloc = alloc;
    (cli)->Identity_verifier = ((unsigned long) 0x6a2785b50a142902ull);
    int idx = Map_Ifaces_put(&cli, &m->ifaces);
    uint32_t handle = m->ifaces.handles[idx];
    Sockaddr_addrFromHandle(&cli->addr, handle);
    Allocator__onFree((alloc), (removeIfaceOnFree), (cli), "AddrIfaceMuxer.c", 112);
    return &cli->iface;
}

struct AddrIfaceMuxer* AddrIfaceMuxer_new(struct Log* log, struct Allocator* alloc)
{
    struct AddrIfaceMuxer_pvt* ctx = Allocator__clone((alloc),((&(struct AddrIfaceMuxer_pvt) { .pub = { .iface.iface.send = incomingFromAddrIf, .iface.alloc = alloc, }, .log = log, .ifaces = { .allocator = alloc }, })),sizeof(*((&(struct AddrIfaceMuxer_pvt) { .pub = { .iface.iface.send = incomingFromAddrIf, .iface.alloc = alloc, }, .log = log, .ifaces = { .allocator = alloc }, }))),"AddrIfaceMuxer.c",118)
       ...
       ;
    (ctx)->Identity_verifier = ((unsigned long) 0x6a2785b50a142902ull);
    return &ctx->pub;
}
