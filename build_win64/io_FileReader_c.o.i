...
# 1 "./util/Linker.h" 1
# 20 "./util/Assert.h" 2
struct Linker_x01428568561367971;
# 33 "./util/Assert.h"
__attribute__((__format__ (__printf__, 1, 2)))
...
struct Linker_x033303208602347145;
...
struct Linker_x08033908356686879;
...
{
    struct FileReader* context = (__extension__ ({ __typeof__((struct FileReader*) reader) Identity_ptr = ((struct FileReader*) reader); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x59a4d92bb92542d8ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("FileReader.c"), (33), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x59a4d92bb92542d8ull))"); } } while (0); Identity_ptr; }));
...
{
    struct FileReader* context = (__extension__ ({ __typeof__((struct FileReader*) reader) Identity_ptr = ((struct FileReader*) reader); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x59a4d92bb92542d8ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("FileReader.c"), (58), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x59a4d92bb92542d8ull))"); } } while (0); Identity_ptr; }));
...
struct Reader* FileReader_new(FILE* toRead, struct Allocator* allocator)
{
    struct FileReader* context = Allocator__clone((allocator),((&(struct FileReader) { .generic = { .read = read, .skip = skip }, .toRead = toRead })),sizeof(*((&(struct FileReader) { .generic = { .read = read, .skip = skip }, .toRead = toRead }))),"FileReader.c",74)
       ...
       ;
    (context)->Identity_verifier = ((unsigned long) 0x59a4d92bb92542d8ull);
