...
# 1 "./util/Linker.h" 1
# 20 "./util/Assert.h" 2
struct Linker_x06271242471616938;
# 33 "./util/Assert.h"
__attribute__((__format__ (__printf__, 1, 2)))
...
struct Linker_x06249522789006647;
...
struct Linker_x038564453721371605;
...
# 1 "./util/Bits.h" 1
# 21 "./util/Bits.h"
struct Linker_x0060147793960480644;
...
struct Linker_x021360785342471145;
...
# 1 "./util/log/Log.h" 1
# 20 "./util/log/Log.h"
struct Linker_x07752108105602156;
...
struct Linker_x06607969932855844;
...
struct Linker_x07890107526233858;
...
# 42 "interface/tuntap/AndroidWrapper.c"
       )); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x4d527f33146f6501ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("AndroidWrapper.c"), (42), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x4d527f33146f6501ull))"); } } while (0); Identity_ptr; })); }));
...
# 57 "interface/tuntap/AndroidWrapper.c"
    ...
    do {             struct Er_Ret* Er_ret = Message_eshift(msg, 4);                          if (Er_ret) {                 struct Er_Ret** Er_errOut = (struct Er_Ret**)0;                 if (Er_errOut) {                     *Er_errOut = Er_unwind("AndroidWrapper.c", 60, Er_ret);                 } else {                     Er__assertFail(Er_unwind("AndroidWrapper.c", 60, Er_ret));                 }             }                  } while (0);
...
# 70 "interface/tuntap/AndroidWrapper.c"
       )); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x4d527f33146f6501ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("AndroidWrapper.c"), (70), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x4d527f33146f6501ull))"); } } while (0); Identity_ptr; })); }));
...
# 74 "interface/tuntap/AndroidWrapper.c"
    ...
    do {             struct Er_Ret* Er_ret = Message_eshift(msg, -4);                          if (Er_ret) {                 struct Er_Ret** Er_errOut = (struct Er_Ret**)0;                 if (Er_errOut) {                     *Er_errOut = Er_unwind("AndroidWrapper.c", 77, Er_ret);                 } else {                     Er__assertFail(Er_unwind("AndroidWrapper.c", 77, Er_ret));                 }             }                  } while (0);
...
struct AndroidWrapper* AndroidWrapper_new(struct Allocator* alloc, struct Log* log)
{
    ...
    (context)->Identity_verifier = ((unsigned long) 0x4d527f33146f6501ull);
