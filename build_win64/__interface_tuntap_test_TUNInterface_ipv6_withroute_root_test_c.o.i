...
# 1 "./util/Linker.h" 1
# 20 "./util/Assert.h" 2
struct Linker_x000299143929329837;
# 33 "./util/Assert.h"
__attribute__((__format__ (__printf__, 1, 2)))
...
struct Linker_x032175827561870696;
...
struct Linker_x06002503972442683;
...
# 21 "./util/Bits.h"
struct Linker_x023921215921643757;
...
struct Linker_x012465113267732675;
...
struct Linker_x0579761479109149;
...
# 22 "./interface/tuntap/TUNInterface.h" 2
# 1 "./util/log/Log.h" 1
# 20 "./util/log/Log.h"
struct Linker_x035073553699262505;
...
void Log_print(struct Log* log,
               ...
               const char* format,
               ...);
# 23 "./interface/tuntap/TUNInterface.h" 2

struct Linker_x046737889233100116;
# 51 "./interface/tuntap/TUNInterface.h"
__attribute__ ((warn_unused_result)) struct Er_Ret* TUNInterface_new(struct Iface* *Er_returnValP, const char* interfaceName, char assignedInterfaceName[256], int isTapMode, struct EventBase* base, struct Log* logger, struct Allocator* alloc)
...
struct Linker_x05312864244611664;
...
struct Linker_x0049703357977453066;
...
struct Linker_x03803511123398122;
...
# 18 "./util/platform/netdev/NetDev.h"
# 1 "./exception/Except.h" 1
# 21 "./exception/Except.h"
struct Linker_x038419034068712343;
...
# 19 "./util/platform/netdev/NetDev.h" 2

# 1 "./util/platform/Sockaddr.h" 1
# 21 "./util/platform/Sockaddr.h"
struct Linker_x046047133325583434;
...
struct Linker_x09185927025857352;
...
# 1 "./util/CString.h" 1
# 21 "./util/CString.h"
struct Linker_x0502782698702329;
...
struct Linker_x07456820959759374;
...
# 33 "./interface/tuntap/test/TUNInterface_ipv6_withroute_root_test.c"
    ...
    struct Iface* tun = (__extension__({             struct Iface* Er_returnVal;             __builtin_memset(&Er_returnVal, 0, sizeof(Er_returnVal));             struct Er_Ret* Er_ret = TUNInterface_new(&Er_returnVal,                        ((void *)0)                       , assignedIfName, 0, base, logger, alloc);                          if (Er_ret) {                 struct Er_Ret** Er_errOut = (struct Er_Ret**)0;                 if (Er_errOut) {                     *Er_errOut = Er_unwind("TUNInterface_ipv6_withroute_root_test.c", 39, Er_ret);                 } else {                     Er__assertFail(Er_unwind("TUNInterface_ipv6_withroute_root_test.c", 39, Er_ret));                 }             }                      Er_returnVal;         }));
    addrA->flags |= (1<<1);
    addrA->prefix = 126;
    do {             struct Er_Ret* Er_ret = NetDev_addAddress(assignedIfName, addrA, logger, alloc);                          if (Er_ret) {                 struct Er_Ret** Er_errOut = (struct Er_Ret**)0;                 if (Er_errOut) {                     *Er_errOut = Er_unwind("TUNInterface_ipv6_withroute_root_test.c", 42, Er_ret);                 } else {                     Er__assertFail(Er_unwind("TUNInterface_ipv6_withroute_root_test.c", 42, Er_ret));                 }             }                  } while (0);
    addrC->flags |= (1<<1);
    addrC->prefix = 125;
    do {             struct Er_Ret* Er_ret = NetDev_setRoutes(assignedIfName, ((struct Sockaddr*[]) { addrC }), 1, logger, alloc);                          if (Er_ret) {                 struct Er_Ret** Er_errOut = (struct Er_Ret**)0;                 if (Er_errOut) {                     *Er_errOut = Er_unwind("TUNInterface_ipv6_withroute_root_test.c", 45, Er_ret);                 } else {                     Er__assertFail(Er_unwind("TUNInterface_ipv6_withroute_root_test.c", 45, Er_ret));                 }             }                  } while (0);
