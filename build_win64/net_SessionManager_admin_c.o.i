...
# 1 "./util/Linker.h" 1
# 20 "./util/Assert.h" 2
struct Linker_x05911739921787238;
# 33 "./util/Assert.h"
__attribute__((__format__ (__printf__, 1, 2)))
...
struct Linker_x09400474151472917;
...
struct Linker_x007496228829092044;
...
# 1 "./util/Bits.h" 1
# 21 "./util/Bits.h"
struct Linker_x01058170904129585;
...
struct Linker_x09658050602635562;
...
struct Linker_x01621565570820589;
...
struct Linker_x05499960601668605;
...
struct Linker_x02845039512056715;
...
# 1 "./util/log/Log.h" 1
# 20 "./util/log/Log.h"
struct Linker_x05944207487837576;
...
struct Linker_x06660215129924774;
...
struct Linker_x040707556066257355;
...
# 1 "./util/CString.h" 1
# 21 "./util/CString.h"
struct Linker_x014558419947923418;
...
struct Linker_x06575221106298106;
...
# 1 "./benc/List.h" 1
# 21 "./benc/List.h"
struct Linker_x013198106003341348;
...
struct Linker_x09934934953177825;
...
struct Linker_x023306910419506432;
...
struct Linker_x045125856188768054;
...
# 1 "./dht/Address.h" 1
# 21 "./dht/Address.h"
struct Linker_x0020098057533490588;
# 34 "./dht/Address.h"
struct Address
{
...
# 21 "./util/ArrayList.h"
struct Linker_x023770884934319025;
...
struct Linker_x04537556462296235;
...
struct Linker_x01475883667638418;
...
struct Linker_x062916513059844;
...
struct Linker_x014171997721206142;
...
struct Linker_x0859264361469088;
...
# 19 "./util/AddrTools.h"
struct Linker_x04590550142833383;
...
{
    struct Context* context = (__extension__ ({ __typeof__((struct Context*) vcontext) Identity_ptr = ((struct Context*) vcontext); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0xa3c33027e0a05344ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("SessionManager_admin.c"), (38), "(Identity_ptr->Identity_verifier == ((unsigned long) 0xa3c33027e0a05344ull))"); } } while (0); Identity_ptr; }));
...
static void sessionStats(Dict* args,
                         ...
                         struct Allocator* alloc)
{
    struct Context* context = (__extension__ ({ __typeof__((struct Context*) vcontext) Identity_ptr = ((struct Context*) vcontext); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0xa3c33027e0a05344ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("SessionManager_admin.c"), (113), "(Identity_ptr->Identity_verifier == ((unsigned long) 0xa3c33027e0a05344ull))"); } } while (0); Identity_ptr; }));
...
static void sessionStatsByIP(Dict* args,
                             ...
                             struct Allocator* alloc)
{
    struct Context* context = (__extension__ ({ __typeof__((struct Context*) vcontext) Identity_ptr = ((struct Context*) vcontext); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0xa3c33027e0a05344ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("SessionManager_admin.c"), (152), "(Identity_ptr->Identity_verifier == ((unsigned long) 0xa3c33027e0a05344ull))"); } } while (0); Identity_ptr; }));
...
static void resetCA(Dict* args,
                    ...
                    struct Allocator* alloc)
{
    struct Context* context = (__extension__ ({ __typeof__((struct Context*) vcontext) Identity_ptr = ((struct Context*) vcontext); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0xa3c33027e0a05344ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("SessionManager_admin.c"), (163), "(Identity_ptr->Identity_verifier == ((unsigned long) 0xa3c33027e0a05344ull))"); } } while (0); Identity_ptr; }));
    struct SessionManager_Session* session = sessionForIP(args, context, txid, alloc);
    if (!session) { return; }
                                   ...
                                   struct Allocator* alloc)
{
    struct Context* ctx = Allocator__clone((alloc),((&(struct Context) { .alloc = alloc, .sm = sm, .admin = admin })),sizeof(*((&(struct Context) { .alloc = alloc, .sm = sm, .admin = admin }))),"SessionManager_admin.c",176)
       ...
       ;
    (ctx)->Identity_verifier = ((unsigned long) 0xa3c33027e0a05344ull);
