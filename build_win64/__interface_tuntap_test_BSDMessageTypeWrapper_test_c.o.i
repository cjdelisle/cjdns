...
# 1 "./util/Linker.h" 1
# 20 "./util/Assert.h" 2
struct Linker_x09992224583355092;
# 33 "./util/Assert.h"
__attribute__((__format__ (__printf__, 1, 2)))
...
struct Linker_x01156461182470061;
...
struct Linker_x09441499946439256;
...
# 1 "./util/Bits.h" 1
# 21 "./util/Bits.h"
struct Linker_x039238149584115267;
...
struct Linker_x0759749075737465;
...
struct Linker_x038389325270302055;
...
# 1 "./util/log/Log.h" 1
# 20 "./util/log/Log.h"
struct Linker_x06396070718810456;
...
struct Linker_x04701406083932569;
...
# 32 "./interface/tuntap/test/BSDMessageTypeWrapper_test.c"
                         )); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x43f14bdea1ea01e0ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("BSDMessageTypeWrapper_test.c"), (32), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x43f14bdea1ea01e0ull))"); } } while (0); Identity_ptr; })); }));
    uint32_t top = (__extension__({             uint32_t Er_returnVal;             __builtin_memset(&Er_returnVal, 0, sizeof(Er_returnVal));             struct Er_Ret* Er_ret = Message_epop32be(&Er_returnVal, msg);                          if (Er_ret) {                 struct Er_Ret** Er_errOut = (struct Er_Ret**)0;                 if (Er_errOut) {                     *Er_errOut = Er_unwind("BSDMessageTypeWrapper_test.c", 33, Er_ret);                 } else {                     Er__assertFail(Er_unwind("BSDMessageTypeWrapper_test.c", 33, Er_ret));                 }             }                      Er_returnVal;         }));
...
# 42 "./interface/tuntap/test/BSDMessageTypeWrapper_test.c"
                         )); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x43f14bdea1ea01e0ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("BSDMessageTypeWrapper_test.c"), (42), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x43f14bdea1ea01e0ull))"); } } while (0); Identity_ptr; })); }));
...
int BSDMessageTypeWrapper_test_main(int argc, char** argv);int BSDMessageTypeWrapper_test_main(int argc, char** argv)
{
    ...
    (ctx)->Identity_verifier = ((unsigned long) 0x43f14bdea1ea01e0ull);
...
# 55 "./interface/tuntap/test/BSDMessageTypeWrapper_test.c"
    ...
    do {             struct Er_Ret* Er_ret = Message_epush32h(msg, ((uint32_t)__builtin_bswap32(((uint32_t)(0x00000800)))));                          if (Er_ret) {                 struct Er_Ret** Er_errOut = (struct Er_Ret**)0;                 if (Er_errOut) {                     *Er_errOut = Er_unwind("BSDMessageTypeWrapper_test.c", 60, Er_ret);                 } else {                     Er__assertFail(Er_unwind("BSDMessageTypeWrapper_test.c", 60, Er_ret));                 }             }                  } while (0);
