...
# 1 "./util/Linker.h" 1
# 20 "./util/Assert.h" 2
struct Linker_x03134609546324709;
# 33 "./util/Assert.h"
__attribute__((__format__ (__printf__, 1, 2)))
...
struct Linker_x09095441556130364;
...
# 1 "./util/log/Log.h" 1
# 20 "./util/log/Log.h"
struct Linker_x019464709171617733;
...
struct Linker_x08406744692901218;
...
struct Linker_x015614563767092626;
...
# 1 "./util/Bits.h" 1
# 21 "./util/Bits.h"
struct Linker_x07911415692530765;
...
static int get(struct RandomSeed* rand, uint64_t buff[8])
{
    ...
    struct DeterminentRandomSeed* drs = (__extension__ ({ __typeof__((struct DeterminentRandomSeed*) rand) Identity_ptr = ((struct DeterminentRandomSeed*) rand); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x892d5874a3665b6full)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("DeterminentRandomSeed.c"), (32), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x892d5874a3665b6full))"); } } while (0); Identity_ptr; }));
...
struct RandomSeed* DeterminentRandomSeed_new(struct Allocator* alloc, uint8_t buff[64])
{
    struct DeterminentRandomSeed* drs = Allocator__clone((alloc),((&(struct DeterminentRandomSeed) { .rs = { .get = get, .name = "DeterminentRandomSeed TESTING ONLY, THIS SHOULD NEVER BE USED IN PRODUCTION!" } })),sizeof(*((&(struct DeterminentRandomSeed) { .rs = { .get = get, .name = "DeterminentRandomSeed TESTING ONLY, THIS SHOULD NEVER BE USED IN PRODUCTION!" } }))),"DeterminentRandomSeed.c",39)
       ...
       ;
    (drs)->Identity_verifier = ((unsigned long) 0x892d5874a3665b6full);
    if (buff) {
