...
# 1 "./util/Linker.h" 1
# 20 "./util/Assert.h" 2
struct Linker_x014822249785938024;
# 33 "./util/Assert.h"
__attribute__((__format__ (__printf__, 1, 2)))
...
struct Linker_x02007091807894199;
...
struct Linker_x09356367451046856;
...
# 1 "./exception/Er.h" 1
# 23 "./exception/Er.h"
struct Linker_x017069678758304163;
...
struct Linker_x01611132554078718;
...
# 1 "./util/log/Log.h" 1
# 20 "./util/log/Log.h"
struct Linker_x021325722216777132;
...
struct Linker_x08763781444752843;
...
struct Linker_x07705413290636132;
...
# 1 "./util/Bits.h" 1
# 21 "./util/Bits.h"
struct Linker_x03589301742221822;
...
struct Linker_x024558218465932402;
...
# 1 "./util/Base10.h" 1
# 21 "./util/Base10.h"
struct Linker_x011461833760888274;
...
# 1 "./util/CString.h" 1
# 21 "./util/CString.h"
struct Linker_x013478717508766525;
...
# 29 "./util/test/Base10_test.c"
    ...
    for (int i = 0; i < 1000; i++) {
        ...
        Message_reset(msg);
        do {             struct Er_Ret* Er_ret = Message_epush(msg,        ((void *)0)       , 32);                          if (Er_ret) {                 struct Er_Ret** Er_errOut = (struct Er_Ret**)0;                 if (Er_errOut) {                     *Er_errOut = Er_unwind("Base10_test.c", 37, Er_ret);                 } else {                     Er__assertFail(Er_unwind("Base10_test.c", 37, Er_ret));                 }             }                  } while (0);
        Message_reset(msg);

        long long num = Random_int64(rand);
        do {             struct Er_Ret* Er_ret = Base10_write(msg, num);                          if (Er_ret) {                 struct Er_Ret** Er_errOut = (struct Er_Ret**)0;                 if (Er_errOut) {                     *Er_errOut = Er_unwind("Base10_test.c", 41, Er_ret);                 } else {                     Er__assertFail(Er_unwind("Base10_test.c", 41, Er_ret));                 }             }                  } while (0);
        ...
        int64_t read = (__extension__({             int64_t Er_returnVal;             __builtin_memset(&Er_returnVal, 0, sizeof(Er_returnVal));             struct Er_Ret* Er_ret = Base10_read(&Er_returnVal, msg);                          if (Er_ret) {                 struct Er_Ret** Er_errOut = (struct Er_Ret**)0;                 if (Er_errOut) {                     *Er_errOut = Er_unwind("Base10_test.c", 48, Er_ret);                 } else {                     Er__assertFail(Er_unwind("Base10_test.c", 48, Er_ret));                 }             }                      Er_returnVal;         }));
