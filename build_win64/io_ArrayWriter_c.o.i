...
# 1 "./util/Linker.h" 1
# 20 "./util/Assert.h" 2
struct Linker_x02359023715031161;
# 33 "./util/Assert.h"
__attribute__((__format__ (__printf__, 1, 2)))
...
struct Linker_x046925258406854686;
...
struct Linker_x02574628635738967;
# 30 "./io/ArrayWriter.h"
...
# 1 "./util/Bits.h" 1
# 21 "./util/Bits.h"
struct Linker_x044862578207463333;
...
{
    struct ArrayWriter_context* context = (__extension__ ({ __typeof__((struct ArrayWriter_context*) writer) Identity_ptr = ((struct ArrayWriter_context*) writer); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x8d03e44fce0dc5e1ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("ArrayWriter.c"), (31), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x8d03e44fce0dc5e1ull))"); } } while (0); Identity_ptr; }));
                               ...
                               struct Allocator* alloc)
{
    struct ArrayWriter_context* context = Allocator__clone((alloc),((&(struct ArrayWriter_context) { .generic = { .write = write }, .beginPointer = writeToBuffer, .pointer = writeToBuffer, .endPointer = (char*) writeToBuffer + length })),sizeof(*((&(struct ArrayWriter_context) { .generic = { .write = write }, .beginPointer = writeToBuffer, .pointer = writeToBuffer, .endPointer = (char*) writeToBuffer + length }))),"ArrayWriter.c",58)
       ...
       ;
    (context)->Identity_verifier = ((unsigned long) 0x8d03e44fce0dc5e1ull);
