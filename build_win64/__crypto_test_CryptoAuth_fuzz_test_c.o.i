...
# 1 "./util/Linker.h" 1
# 20 "./util/Assert.h" 2
struct Linker_x03615541787625507;
# 33 "./util/Assert.h"
__attribute__((__format__ (__printf__, 1, 2)))
...
struct Linker_x003857355288120612;
...
# 1 "./exception/Er.h" 1
# 23 "./exception/Er.h"
struct Linker_x048288252075292615;
...
struct Linker_x03064440956155299;
...
# 1 "./util/log/Log.h" 1
# 20 "./util/log/Log.h"
struct Linker_x03517792820964356;
...
struct Linker_x07413477360566083;
...
struct Linker_x07664681819136676;
...
# 1 "./util/CString.h" 1
# 21 "./util/CString.h"
struct Linker_x04080220603182647;
...
struct Linker_x010481851866396341;
...
# 21 "./util/ArrayList.h"
struct Linker_x045679047780803583;
...
# 1 "./util/Bits.h" 1
# 21 "./util/Bits.h"
struct Linker_x0793064901706428;
...
struct Linker_x0666231644322195;
...
# 1 "./wire/Message.h" 1
# 23 "./wire/Message.h"
struct Linker_x08339239313033335;
...
struct Linker_x06785076239021559;
...
struct Linker_x08097161085504916;
...
# 1 "./util/Hex.h" 1
# 21 "./util/Hex.h"
struct Linker_x03062882051637572;
# 32 "./util/Hex.h"
...
struct Linker_x07189393822780106;
...
struct Linker_x04128826873129523;
...
struct Linker_x03728061705242276;
...
static void delay(struct Context* ctx, struct Node* from, struct Message* msg, int afterMsgs)
{
    ...
    if (from == &ctx->nodeA) { do { if ((ctx)->log && Log_Level_DEBUG >= Log_Level_DEBUG) { Log_print((ctx)->log, Log_Level_DEBUG, "CryptoAuth_fuzz_test.c", 167, "(nodeA) " "DELAY %d packets (ptr:%p)", afterMsgs, msg); } } while (0); } else { do { if ((ctx)->log && Log_Level_DEBUG >= Log_Level_DEBUG) { Log_print((ctx)->log, Log_Level_DEBUG, "CryptoAuth_fuzz_test.c", 167, "(nodeB) " "DELAY %d packets (ptr:%p)", afterMsgs, msg); } } while (0); };
...
# 238 "./crypto/test/CryptoAuth_fuzz_test.c"
    ...
    if (fromNode == &ctx->nodeA) { do { if ((ctx)->log && Log_Level_DEBUG >= Log_Level_DEBUG) { Log_print((ctx)->log, Log_Level_DEBUG, "CryptoAuth_fuzz_test.c", 243, "(nodeA) " "SENDING QUEUED (ptr:%p)", dmsg->msg); } } while (0); } else { do { if ((ctx)->log && Log_Level_DEBUG >= Log_Level_DEBUG) { Log_print((ctx)->log, Log_Level_DEBUG, "CryptoAuth_fuzz_test.c", 243, "(nodeB) " "SENDING QUEUED (ptr:%p)", dmsg->msg); } } while (0); };



    sendFrom(ctx, fromNode, dmsg->msg);
    Allocator__free((dmsg->alloc),"CryptoAuth_fuzz_test.c",248);
    return 
# 249 "./crypto/test/CryptoAuth_fuzz_test.c" 3 4
          1
# 249 "./crypto/test/CryptoAuth_fuzz_test.c"
              ;
}
...
static void mainLoop(struct Context* ctx)
{
    for (int i = 0; i < 500000; i++) {
        ...
        struct Allocator* alloc = Allocator__child((ctx->alloc),"CryptoAuth_fuzz_test.c",267);
        struct Message* msg = Message_new(0, 512, alloc);
        do {             struct Er_Ret* Er_ret = Message_epush(msg, "hey", 4);                          if (Er_ret) {                 struct Er_Ret** Er_errOut = (struct Er_Ret**)0;                 if (Er_errOut) {                     *Er_errOut = Er_unwind("CryptoAuth_fuzz_test.c", 269, Er_ret);                 } else {                     Er__assertFail(Er_unwind("CryptoAuth_fuzz_test.c", 269, Er_ret));                 }             }                  } while (0);
        do { if (!((!CryptoAuth_encrypt(ctx->nodeA.session, msg)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("CryptoAuth_fuzz_test.c"), (270), "(!CryptoAuth_encrypt(ctx->nodeA.session, msg))"); } } while (0);
        sendFrom(ctx, &ctx->nodeA, msg);
        Allocator__free((alloc),"CryptoAuth_fuzz_test.c",272);
    }
...
void* CryptoAuth_fuzz_test_FUZZ_INIT(struct Allocator* alloc, struct Random* rand)
{
    struct Context* ctx = Allocator__calloc((alloc),(sizeof(struct Context)),(1),"CryptoAuth_fuzz_test.c",280);
    (ctx)->Identity_verifier = ((unsigned long) 0xf07748a1484d2a7cull);
    struct EventBase* base = EventBase_new(alloc);
    ctx->alloc = alloc;
    ctx->nodeA.ca = CryptoAuth_new(alloc, 
# 284 "./crypto/test/CryptoAuth_fuzz_test.c" 3 4
                                         ((void *)0)
# 284 "./crypto/test/CryptoAuth_fuzz_test.c"
                                             , base, 
# 284 "./crypto/test/CryptoAuth_fuzz_test.c" 3 4
                                                     ((void *)0)
# 284 "./crypto/test/CryptoAuth_fuzz_test.c"
...
# 285 "./crypto/test/CryptoAuth_fuzz_test.c" 3 4
                                         ((void *)0)
# 285 "./crypto/test/CryptoAuth_fuzz_test.c"
                                             , base, 
# 285 "./crypto/test/CryptoAuth_fuzz_test.c" 3 4
                                                     ((void *)0)
# 285 "./crypto/test/CryptoAuth_fuzz_test.c"
...
void CryptoAuth_fuzz_test_FUZZ_MAIN(void* vctx, struct Message* fuzz)
{
    struct Context* ctx = (__extension__ ({ __typeof__((struct Context*) vctx) Identity_ptr = ((struct Context*) vctx); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0xf07748a1484d2a7cull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("CryptoAuth_fuzz_test.c"), (295), "(Identity_ptr->Identity_verifier == ((unsigned long) 0xf07748a1484d2a7cull))"); } } while (0); Identity_ptr; }));


    struct RandomSeed* rs = DeterminentRandomSeed_new(ctx->alloc, fuzz->bytes);
    ctx->rand = Random_newWithSeed(ctx->alloc, 
# 299 "./crypto/test/CryptoAuth_fuzz_test.c" 3 4
                                              ((void *)0)
# 299 "./crypto/test/CryptoAuth_fuzz_test.c"
                                                  , rs, 
# 299 "./crypto/test/CryptoAuth_fuzz_test.c" 3 4
                                                        ((void *)0)
# 299 "./crypto/test/CryptoAuth_fuzz_test.c"
...
# 302 "./crypto/test/CryptoAuth_fuzz_test.c" 3 4
                                                                           ((void *)0)
# 302 "./crypto/test/CryptoAuth_fuzz_test.c"
                                                                               , ctx->nodeB.ca);
    } else {
...
# 319 "./crypto/test/CryptoAuth_fuzz_test.c" 3 4
                                                ((void *)0)
# 319 "./crypto/test/CryptoAuth_fuzz_test.c"
                                                    , ctx->nodeA.session);
    }
