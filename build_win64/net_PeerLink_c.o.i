...
# 1 "./util/Linker.h" 1
# 20 "./util/Assert.h" 2
struct Linker_x0044950035741840866;
# 33 "./util/Assert.h"
__attribute__((__format__ (__printf__, 1, 2)))
...
struct Linker_x06912721372555795;
...
# 1 "./exception/Er.h" 1
# 23 "./exception/Er.h"
struct Linker_x07175052204486692;
...
# 1 "./util/Bits.h" 1
# 21 "./util/Bits.h"
struct Linker_x06571607424024923;
...
struct Linker_x09600844387441168;
...
struct Linker_x02598724401582042;
...
struct Linker_x08357337059879966;
...
struct Linker_x06042054891376387;
...
struct Linker_x06495436902935068;
...
# 21 "./util/ArrayList.h"
struct Linker_x03301316179372522;
...
{
    struct PeerLink_pvt* pl = (__extension__ ({ __typeof__((struct PeerLink_pvt*) peerLink) Identity_ptr = ((struct PeerLink_pvt*) peerLink); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x265b501f008a3115ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("PeerLink.c"), (39), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x265b501f008a3115ull))"); } } while (0); Identity_ptr; }));
...
{
    struct PeerLink_pvt* pl = (__extension__ ({ __typeof__((struct PeerLink_pvt*) peerLink) Identity_ptr = ((struct PeerLink_pvt*) peerLink); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x265b501f008a3115ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("PeerLink.c"), (49), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x265b501f008a3115ull))"); } } while (0); Identity_ptr; }));
...
{
    struct PeerLink_pvt* pl = (__extension__ ({ __typeof__((struct PeerLink_pvt*) peerLink) Identity_ptr = ((struct PeerLink_pvt*) peerLink); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x265b501f008a3115ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("PeerLink.c"), (57), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x265b501f008a3115ull))"); } } while (0); Identity_ptr; }));
    Kbps_accumulate(&pl->recvBw, Time_currentTimeMilliseconds(pl->base), msg->length);
}

void PeerLink_kbps(struct PeerLink* peerLink, struct PeerLink_Kbps* output)
{
    struct PeerLink_pvt* pl = (__extension__ ({ __typeof__((struct PeerLink_pvt*) peerLink) Identity_ptr = ((struct PeerLink_pvt*) peerLink); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x265b501f008a3115ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("PeerLink.c"), (63), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x265b501f008a3115ull))"); } } while (0); Identity_ptr; }));
...
struct PeerLink* PeerLink_new(struct EventBase* base, struct Allocator* allocator)
{
    ...
    (pl)->Identity_verifier = ((unsigned long) 0x265b501f008a3115ull);
    pl->base = base;
    pl->alloc = alloc;
