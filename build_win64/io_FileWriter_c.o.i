...
# 1 "./util/Linker.h" 1
# 20 "./util/Assert.h" 2
struct Linker_x0818616612727922;
# 33 "./util/Assert.h"
__attribute__((__format__ (__printf__, 1, 2)))
...
struct Linker_x0652565373120304;
...
struct Linker_x08407959948217008;
...
{
    struct FileWriter_context* context = (__extension__ ({ __typeof__((struct FileWriter_context*) writer) Identity_ptr = ((struct FileWriter_context*) writer); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x287fe0ff943ac70dull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("FileWriter.c"), (28), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x287fe0ff943ac70dull))"); } } while (0); Identity_ptr; }));
...
struct Writer* FileWriter_new(FILE* writeTo, struct Allocator* allocator)
{
    struct FileWriter_context* context = Allocator__clone((allocator),((&(struct FileWriter_context) { .generic = { .write = write }, .writeTo = writeTo })),sizeof(*((&(struct FileWriter_context) { .generic = { .write = write }, .writeTo = writeTo }))),"FileWriter.c",41)
       ...
       ;
    (context)->Identity_verifier = ((unsigned long) 0x287fe0ff943ac70dull);
