...
# 1 "./util/Linker.h" 1
# 20 "./util/Assert.h" 2
struct Linker_x020264641330453004;
# 33 "./util/Assert.h"
__attribute__((__format__ (__printf__, 1, 2)))
...
struct Linker_x049705119608099935;
...
struct Linker_x016291703489444687;
...
# 1 "./util/Bits.h" 1
# 21 "./util/Bits.h"
struct Linker_x08518309820336882;
...
struct Linker_x07793191181485717;
...
struct Linker_x08054970349531387;
...
struct Linker_x04446421209193321;
...
struct Linker_x04084630090651302;
...
# 1 "./util/log/Log.h" 1
# 20 "./util/log/Log.h"
struct Linker_x032953138578444263;
...
struct Linker_x06904273340601872;
...
struct Linker_x012049719576390583;
...
# 1 "./benc/List.h" 1
# 21 "./benc/List.h"
struct Linker_x08109646625907823;
...
# 1 "./util/CString.h" 1
# 21 "./util/CString.h"
struct Linker_x08408362141619412;
...
struct Linker_x06088167967926454;
...
struct Linker_x019999495832408187;
...
struct Linker_x09385689965100543;
...
struct Linker_x049856379584724664;
...
# 1 "./dht/Address.h" 1
# 21 "./dht/Address.h"
struct Linker_x01696784980792594;
# 34 "./dht/Address.h"
struct Address
{
...
struct Linker_x04498273314272725;
...
struct Linker_x05463700576418131;
...
struct Linker_x09763678452693747;
...
struct Linker_x03463749931134501;
...
struct Linker_x048578764390552376;
...
struct Linker_x05257190029229679;
...
struct Linker_x03799053026903705;
...
struct Linker_x05726816418486933;
...
struct Linker_x00485720890514767;
...
{
    struct Context* ctx = (__extension__ ({ __typeof__((struct Context*) vcontext) Identity_ptr = ((struct Context*) vcontext); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x8f98d29d6cc68813ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("SupernodeHunter_admin.c"), (46), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x8f98d29d6cc68813ull))"); } } while (0); Identity_ptr; }));
...
{
    struct Context* ctx = (__extension__ ({ __typeof__((struct Context*) vcontext) Identity_ptr = ((struct Context*) vcontext); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x8f98d29d6cc68813ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("SupernodeHunter_admin.c"), (72), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x8f98d29d6cc68813ull))"); } } while (0); Identity_ptr; }));
    struct Address* addr = getAddr(args, requestAlloc);
    if (!addr) {
        sendError(ctx, txid, requestAlloc, "parse_error");
        return;
...
{
    struct Context* ctx = (__extension__ ({ __typeof__((struct Context*) vcontext) Identity_ptr = ((struct Context*) vcontext); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x8f98d29d6cc68813ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("SupernodeHunter_admin.c"), (100), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x8f98d29d6cc68813ull))"); } } while (0); Identity_ptr; }));
    int page = 0;
...
{
    struct Context* ctx = (__extension__ ({ __typeof__((struct Context*) vcontext) Identity_ptr = ((struct Context*) vcontext); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x8f98d29d6cc68813ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("SupernodeHunter_admin.c"), (119), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x8f98d29d6cc68813ull))"); } } while (0); Identity_ptr; }));
                                    ...
                                    struct Allocator* alloc)
{
    struct Context* ctx = Allocator__clone((alloc),((&(struct Context) { .admin = admin, .alloc = alloc, .snh = snh })),sizeof(*((&(struct Context) { .admin = admin, .alloc = alloc, .snh = snh }))),"SupernodeHunter_admin.c",136)
       ...
       ;
    (ctx)->Identity_verifier = ((unsigned long) 0x8f98d29d6cc68813ull);
