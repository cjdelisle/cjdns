...
# 1 "<command-line>"
# 1 "admin/angel/InterfaceWaiter.c"
# 16 "admin/angel/InterfaceWaiter.c"
...
# 1 "./util/Linker.h" 1
# 20 "./util/Assert.h" 2
struct Linker_x09774532513577832;
# 33 "./util/Assert.h"
__attribute__((__format__ (__printf__, 1, 2)))
...
struct Linker_x08162106199964514;
...
# 1 "./exception/Er.h" 1
# 23 "./exception/Er.h"
struct Linker_x05740250092796548;
...
struct Linker_x03747514932845968;
...
# 21 "./util/Bits.h"
struct Linker_x019086072959351896;
...
struct Linker_x05728193216114503;
...
struct Linker_x02354796956900369;
...
struct Linker_x05545592797060033;
...
# 1 "./util/log/Log.h" 1
# 20 "./util/log/Log.h"
struct Linker_x007790976604016175;
...
struct Linker_x05444958208063329;
...
struct Linker_x01971524093029804;
...
{
    struct Context* ctx = (__extension__ ({ __typeof__((struct Context*) iface) Identity_ptr = ((struct Context*) iface); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x4845ac200cd6f620ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("InterfaceWaiter.c"), (46), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x4845ac200cd6f620ull))"); } } while (0); Identity_ptr; }));
                                            ...
                                            struct Except* eh)
{
    ...
    (&ctx)->Identity_verifier = ((unsigned long) 0x4845ac200cd6f620ull);
    Iface_plumb(iface, &ctx.iface);

    struct Allocator* tempAlloc = Allocator__child((alloc),"InterfaceWaiter.c",69);
    ctx.timeout = Timeout__setTimeout((timeout), (&ctx), (10000), (eventBase), (tempAlloc), "InterfaceWaiter.c", 70);
