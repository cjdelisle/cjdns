...
# 1 "./util/Linker.h" 1
# 20 "./util/Assert.h" 2
struct Linker_x030017534900978915;
# 33 "./util/Assert.h"
__attribute__((__format__ (__printf__, 1, 2)))
...
struct Linker_x09994967898872362;
...
struct Linker_x021629947146719042;
...
# 1 "./util/Bits.h" 1
# 21 "./util/Bits.h"
struct Linker_x020058977533053612;
...
struct Linker_x0498431503605526;
...
struct Linker_x05961795671268248;
...
struct Linker_x0004185069534509589;
...
# 1 "./util/CString.h" 1
# 21 "./util/CString.h"
struct Linker_x01125215669594315;
...
struct Linker_x05761681001994405;
...
# 1 "./benc/Dict.h" 1
# 21 "./benc/Dict.h"
struct Linker_x07127148427561445;
...
# 1 "./util/log/Log.h" 1
# 20 "./util/log/Log.h"
struct Linker_x09840499934875016;
...
struct Linker_x09662607081816617;
...
struct Linker_x06460961958435278;
...
struct Linker_x06170597452357107;
...
struct Linker_x07355510273249541;
...
# 1 "./benc/List.h" 1
# 21 "./benc/List.h"
struct Linker_x017476176184750414;
...
struct Linker_x044793903241021127;
...
# 1 "./util/Hex.h" 1
# 21 "./util/Hex.h"
struct Linker_x09187838026515103;
# 32 "./util/Hex.h"
...
struct Linker_x06539734253019045;
...
# 82 "client/AdminClient.c"
    ...
    do {             struct Er_Ret* Er_ret = BencMessageWriter_write(message, msg);                          if (Er_ret) {                 struct Er_Ret** Er_errOut = (struct Er_Ret**)0;                 if (Er_errOut) {                     *Er_errOut = Er_unwind("AdminClient.c", 93, Er_ret);                 } else {                     Er__assertFail(Er_unwind("AdminClient.c", 93, Er_ret));                 }             }                  } while (0);
...
# 118 "client/AdminClient.c"
                         )); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0xe8e5ce473ce61745ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("AdminClient.c"), (118), "(Identity_ptr->Identity_verifier == ((unsigned long) 0xe8e5ce473ce61745ull))"); } } while (0); Identity_ptr; })); }));

    struct Sockaddr_storage source;
    do {             struct Er_Ret* Er_ret = Message_epop(msg, &source, ctx->targetAddr->addrLen);                          if (Er_ret) {                 struct Er_Ret** Er_errOut = (struct Er_Ret**)0;                 if (Er_errOut) {                     *Er_errOut = Er_unwind("AdminClient.c", 121, Er_ret);                 } else {                     Er__assertFail(Er_unwind("AdminClient.c", 121, Er_ret));                 }             }                  } while (0);
...
# 136 "client/AdminClient.c"
                         ; }
    do {             struct Er_Ret* Er_ret = Message_eshift(msg, origLen);                          if (Er_ret) {                 struct Er_Ret** Er_errOut = (struct Er_Ret**)0;                 if (Er_errOut) {                     *Er_errOut = Er_unwind("AdminClient.c", 137, Er_ret);                 } else {                     Er__assertFail(Er_unwind("AdminClient.c", 137, Er_ret));                 }             }                  } while (0);
...
{
    struct Request* req = (__extension__ ({ __typeof__((struct Request*) job->userData) Identity_ptr = ((struct Request*) job->userData); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0xe8e5ce473ce61745ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("AdminClient.c"), (165), "(Identity_ptr->Identity_verifier == ((unsigned long) 0xe8e5ce473ce61745ull))"); } } while (0); Identity_ptr; }));
                               ...
                               AdminClient_RespHandler callback)
{
    ...
    struct Request* req = Allocator__clone((reqAlloc),((&(struct Request) { .alloc = reqAlloc, .ctx = ctx, .promise = promise })),sizeof(*((&(struct Request) { .alloc = reqAlloc, .ctx = ctx, .promise = promise }))),"AdminClient.c",180)
       ...
       ;
    (req)->Identity_verifier = ((unsigned long) 0xe8e5ce473ce61745ull);
...
# 190 "client/AdminClient.c"
    ...
    do {             struct Er_Ret* Er_ret = BencMessageWriter_write(messageDict, msg);                          if (Er_ret) {                 struct Er_Ret** Er_errOut = (struct Er_Ret**)0;                 if (Er_errOut) {                     *Er_errOut = Er_unwind("AdminClient.c", 200, Er_ret);                 } else {                     Er__assertFail(Er_unwind("AdminClient.c", 200, Er_ret));                 }             }                  } while (0);
    ...
    do {             struct Er_Ret* Er_ret = Message_epush(msg, ctx->targetAddr, ctx->targetAddr->addrLen);                          if (Er_ret) {                 struct Er_Ret** Er_errOut = (struct Er_Ret**)0;                 if (Er_errOut) {                     *Er_errOut = Er_unwind("AdminClient.c", 212, Er_ret);                 } else {                     Er__assertFail(Er_unwind("AdminClient.c", 212, Er_ret));                 }             }                  } while (0);

    Iface_send(&ctx->addrIface, msg);
    Allocator__free((child),"AdminClient.c",215);
...
static void requestCallback(struct Request* req)
{
    ...
    Allocator__free((req->promise->alloc),"AdminClient.c",225);
}
...
struct AdminClient_Promise* AdminClient_rpcCall(String* function,
                                                ...
                                                struct Allocator* alloc)
{
    struct Context* ctx = (__extension__ ({ __typeof__((struct Context*) client) Identity_ptr = ((struct Context*) client); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0xe8e5ce473ce61745ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("AdminClient.c"), (267), "(Identity_ptr->Identity_verifier == ((unsigned long) 0xe8e5ce473ce61745ull))"); } } while (0); Identity_ptr; }));
    Dict a = (args) ? *args : 
...
# 269 "client/AdminClient.c" 3 4
                  ((void *)0) 
# 269 "client/AdminClient.c"
                  }) }) })
...
# 319 "client/AdminClient.c"
    ...
    if (Sockaddr_getFamily(context->targetAddr) == Sockaddr_AF_INET) {
        ...
        if (Bits_isZero(addrBytes, len)) {
            ...
            Bits__memcpy(addrBytes,&loopback,4,"AdminClient.c",330);
        }
    }
    do { if (logger && Log_Level_DEBUG >= Log_Level_DEBUG) { Log_print(logger, Log_Level_DEBUG, "AdminClient.c", 333, "Connecting to [%s]", Sockaddr_print(context->targetAddr, alloc)); } } while (0);
