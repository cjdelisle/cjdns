...
# 1 "./util/Linker.h" 1
# 20 "./util/Assert.h" 2
struct Linker_x07661875417000583;
# 33 "./util/Assert.h"
__attribute__((__format__ (__printf__, 1, 2)))
...
struct Linker_x06402434099404846;
...
struct Linker_x042397075437044407;
...
# 1 "./util/Bits.h" 1
# 21 "./util/Bits.h"
struct Linker_x0574605722180642;
...
struct Linker_x016513621950500235;
...
struct Linker_x0653938181837737;
...
struct Linker_x06454258122334882;
...
struct Linker_x005043352270710666;
...
# 1 "./util/log/Log.h" 1
# 20 "./util/log/Log.h"
struct Linker_x09646772809931481;
...
struct Linker_x07924569076267436;
...
struct Linker_x031436699471162455;
...
# 1 "./util/CString.h" 1
# 21 "./util/CString.h"
struct Linker_x09656502514281666;
...
struct Linker_x06992648853699899;
...
struct Linker_x08837654838772193;
...
struct Linker_x0993549487342891;
...
# 1 "./util/Hex.h" 1
# 21 "./util/Hex.h"
struct Linker_x0671836428814037;
# 32 "./util/Hex.h"
...
struct Linker_x05623681395680975;
...
struct Linker_x00742577235679569;
...
static void sendMessage(struct Message* message, struct Sockaddr* dest, struct Admin_pvt* admin)
{
    ...
    do {             struct Er_Ret* Er_ret = Message_epush(message, dest, dest->addrLen);                          if (Er_ret) {                 struct Er_Ret** Er_errOut = (struct Er_Ret**)0;                 if (Er_errOut) {                     *Er_errOut = Er_unwind("Admin.c", 120, Er_ret);                 } else {                     Er__assertFail(Er_unwind("Admin.c", 120, Er_ret));                 }             }                  } while (0);
                     ...
                     int fd)
{
    ...
    do {             struct Er_Ret* Er_ret = BencMessageWriter_write(message, admin->tempSendMsg);                          if (Er_ret) {                 struct Er_Ret** Er_errOut = (struct Er_Ret**)0;                 if (Er_errOut) {                     *Er_errOut = Er_unwind("Admin.c", 131, Er_ret);                 } else {                     Er__assertFail(Er_unwind("Admin.c", 131, Er_ret));                 }             }                  } while (0);
    struct Message* msg = Message_new(0, admin->tempSendMsg->length + 32, alloc);
    do {             struct Er_Ret* Er_ret = Message_epush(msg, admin->tempSendMsg->bytes, admin->tempSendMsg->length);                          if (Er_ret) {                 struct Er_Ret** Er_errOut = (struct Er_Ret**)0;                 if (Er_errOut) {                     *Er_errOut = Er_unwind("Admin.c", 133, Er_ret);                 } else {                     Er__assertFail(Er_unwind("Admin.c", 133, Er_ret));                 }             }                  } while (0);
...
# 147 "admin/Admin.c" 3 4
                                                        9223372036854775807LL
# 147 "admin/Admin.c"
                                                                 )) {
        Allocator__free((admin->map.values[index]->allocator),"Admin.c",148);
...
{
    struct Admin_pvt* admin = (__extension__ ({ __typeof__((struct Admin_pvt*) vAdmin) Identity_ptr = ((struct Admin_pvt*) vAdmin); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x3346e23044e40dccull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Admin.c"), (158), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x3346e23044e40dccull))"); } } while (0); Identity_ptr; }));
    ...
    do { if (admin->logger && Log_Level_DEBUG >= Log_Level_DEBUG) { Log_print(admin->logger, Log_Level_DEBUG, "Admin.c", 166, "Cleared [%d] expired sessions", count); } } while (0);
}

static int sendMessage0(Dict* message, String* txid, struct Admin* adminPub, int fd)
{
    struct Admin_pvt* admin = (__extension__ ({ __typeof__((struct Admin_pvt*) adminPub) Identity_ptr = ((struct Admin_pvt*) adminPub); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x3346e23044e40dccull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Admin.c"), (171), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x3346e23044e40dccull))"); } } while (0); Identity_ptr; }));
    if (!admin) {
        return 0;
    }
    do { if (!((txid && txid->len >= sizeof(struct Sockaddr)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Admin.c"), (175), "(txid && txid->len >= sizeof(struct Sockaddr))"); } } while (0);
    uint16_t addrLen = 0;
    Bits__memcpy(&addrLen,txid->bytes,2,"Admin.c",177);
    do { if (!((txid->len >= addrLen))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Admin.c"), (178), "(txid->len >= addrLen)"); } } while (0);

    struct Allocator* alloc = 
# 180 "admin/Admin.c" 3 4
                             ((void *)0)
# 180 "admin/Admin.c"
                                 ;
    if (admin->currentRequest) {
        ...
        alloc = Allocator__child((admin->allocator),"Admin.c",184);
    }
    ...
    if (!admin->currentRequest) {
        Allocator__free((alloc),"Admin.c",211);
    }
                      ...
                      struct Admin_pvt* admin)
{
    ...
    String* error = 
# 262 "admin/Admin.c" 3 4
                   ((void *)0)
# 262 "admin/Admin.c"
                       ;
    while (entry != 
# 263 "admin/Admin.c" 3 4
                   ((void *)0)
# 263 "admin/Admin.c"
                       ) {
        String* key = (String*) entry->key;
        do { if (!((entry->val->type == Object_DICT))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Admin.c"), (265), "(entry->val->type == Object_DICT)"); } } while (0);
...
{
    struct Admin_pvt* admin = (__extension__ ({ __typeof__((struct Admin_pvt*) vAdmin) Identity_ptr = ((struct Admin_pvt*) vAdmin); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x3346e23044e40dccull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Admin.c"), (293), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x3346e23044e40dccull))"); } } while (0); Identity_ptr; }));
...
{
    struct Admin_pvt* admin = (__extension__ ({ __typeof__((struct Admin_pvt*) vAdmin) Identity_ptr = ((struct Admin_pvt*) vAdmin); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x3346e23044e40dccull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Admin.c"), (302), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x3346e23044e40dccull))"); } } while (0); Identity_ptr; }));
                          ...
                          struct Admin_pvt* admin)
{
    ...
    if (!query) {
        do { if (admin->logger && Log_Level_INFO >= Log_Level_DEBUG) { Log_print(admin->logger, Log_Level_INFO, "Admin.c", 328, "Got a non-query from admin interface"); } } while (0);
        return;
    }
...
# 335 "admin/Admin.c" 3 4
                                   ((void *)0)
# 335 "admin/Admin.c"
                                       , txidlen, allocator);
    Bits__memcpy(txid->bytes,src,src->addrLen,"Admin.c",336);
    if (userTxid) {
        Bits__memcpy(txid->bytes + src->addrLen,userTxid->bytes,userTxid->len,"Admin.c",338);
    }
...
# 365 "admin/Admin.c"
    ...
    if (index >= 0) {
        ...
        struct Allocator* entryAlloc = Allocator__child((admin->allocator),"Admin.c",376);
        struct MapValue* mv = Allocator__calloc((entryAlloc),(sizeof(struct MapValue)),(1),"Admin.c",377);
    ...
    if (String_equals(admin->password, (&(String) { .bytes = "NONE", .len = CString_strlen("NONE") }))) {
                          ...
                          struct Admin_pvt* admin)
{
    if (0) {
        ...
        do { if (admin->logger && Log_Level_KEYS >= Log_Level_DEBUG) { Log_print(admin->logger, Log_Level_KEYS, "Admin.c", 425, "Got message from [%s] [%s]", Sockaddr_print(src, alloc), message->bytes); } } while (0)
                                                            ;
    ...
    if (message->length > 512) {
        ...
        Bits__memcpy(message->bytes,"d5:error16:Request too big.e",(sizeof("d5:error16:Request too big.e") - 1),"Admin.c",434);
    ...
    Dict* messageDict = 
# 441 "admin/Admin.c" 3 4
                       ((void *)0)
# 441 "admin/Admin.c"
                           ;
    const char* err = BencMessageReader_readNoExcept(message, alloc, &messageDict);
    if (err) {
        do { if (admin->logger && Log_Level_WARN >= Log_Level_DEBUG) { Log_print(admin->logger, Log_Level_WARN, "Admin.c", 444, "Unparsable data from [%s] content: [%s] error: [%s]", Sockaddr_print(src, alloc), Hex_print(message->bytes, message->length, alloc), err); } } while (0)
    ...
    if (message->length) {
        do { if (admin->logger && Log_Level_WARN >= Log_Level_DEBUG) { Log_print(admin->logger, Log_Level_WARN, "Admin.c", 453, "Message from [%s] contained garbage after byte [%d] content: [%s]", Sockaddr_print(src, alloc), message->length, message->bytes); } } while (0)
    ...
    do {             struct Er_Ret* Er_ret = Message_eshift(message, origMessageLen);                          if (Er_ret) {                 struct Er_Ret** Er_errOut = (struct Er_Ret**)0;                 if (Er_errOut) {                     *Er_errOut = Er_unwind("Admin.c", 460, Er_ret);                 } else {                     Er__assertFail(Er_unwind("Admin.c", 460, Er_ret));                 }             }                  } while (0);
...
# 467 "admin/Admin.c"
                             )); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x3346e23044e40dccull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Admin.c"), (467), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x3346e23044e40dccull))"); } } while (0); Identity_ptr; })); }));
    struct Allocator* alloc = Allocator__child((admin->allocator),"Admin.c",468);
    struct Sockaddr* addrPtr = (__extension__({             struct Sockaddr* Er_returnVal;             __builtin_memset(&Er_returnVal, 0, sizeof(Er_returnVal));             struct Er_Ret* Er_ret = AddrIface_popAddr(&Er_returnVal, message);                          if (Er_ret) {                 struct Er_Ret** Er_errOut = (struct Er_Ret**)0;                 if (Er_errOut) {                     *Er_errOut = Er_unwind("Admin.c", 469, Er_ret);                 } else {                     Er__assertFail(Er_unwind("Admin.c", 469, Er_ret));                 }             }                      Er_returnVal;         }));

    admin->currentRequest = message;
    handleMessage(message, Sockaddr_clone(addrPtr, alloc), alloc, admin);

    admin->currentRequest = 
# 474 "admin/Admin.c" 3 4
                           ((void *)0)
# 474 "admin/Admin.c"
                               ;
    Allocator__free((alloc),"Admin.c",475);
    return 
# 476 "admin/Admin.c" 3 4
          ((void *)0)
# 476 "admin/Admin.c"
              ;
}
...
{
    struct Admin_pvt* admin = (__extension__ ({ __typeof__((struct Admin_pvt*) adminPub) Identity_ptr = ((struct Admin_pvt*) adminPub); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x3346e23044e40dccull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Admin.c"), (487), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x3346e23044e40dccull))"); } } while (0); Identity_ptr; }));

    String* str = String_new(name, admin->allocator);
    admin->functions =
        Allocator__realloc((admin->allocator),(admin->functions),(sizeof(struct Function) * (admin->functionCount + 1)),"Admin.c",491)
...
{
    struct Admin_pvt* admin = (__extension__ ({ __typeof__((struct Admin_pvt*) vAdmin) Identity_ptr = ((struct Admin_pvt*) vAdmin); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x3346e23044e40dccull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Admin.c"), (526), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x3346e23044e40dccull))"); } } while (0); Identity_ptr; }));
    ...
    if (fd < 0) {
        ...
        Dict_putInt(res, String_new("fd", requestAlloc), fd, requestAlloc);
    }
    Dict_putString(res, String_new("error", requestAlloc), String_new(error, requestAlloc), requestAlloc);
    Admin_sendMessage(res, txid, &admin->pub);
}

static void exportFd(Dict* args, void* vAdmin, String* txid, struct Allocator* requestAlloc)
{
    struct Admin_pvt* admin = (__extension__ ({ __typeof__((struct Admin_pvt*) vAdmin) Identity_ptr = ((struct Admin_pvt*) vAdmin); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x3346e23044e40dccull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Admin.c"), (545), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x3346e23044e40dccull))"); } } while (0); Identity_ptr; }));
    int64_t* fd_p = Dict_getInt(args, (&(String) { .bytes = "fd", .len = CString_strlen("fd") }));
    if (!fd_p || *fd_p < 0) {
        Dict* res = Dict_new(requestAlloc);
        Dict_putString(res, String_new("error", requestAlloc), String_new("invalid fd", requestAlloc), requestAlloc);
        Admin_sendMessage(res, txid, &admin->pub);
        return;
    }
    int fd = *fd_p;
                        ...
                        String* password)
{
    ...
    struct Admin_pvt* admin = Allocator__calloc((alloc),(sizeof(struct Admin_pvt)),(1),"Admin.c",573);
    (admin)->Identity_verifier = ((unsigned long) 0x3346e23044e40dccull);
...
# 583 "admin/Admin.c" 3 4
                     ((void *)0)
# 583 "admin/Admin.c"
                     );

    Timeout__setInterval((clearExpiredAddresses), (admin), (30000 * 3), (eventBase), (alloc), "Admin.c", 585);

    Admin_registerFunctionWithArgCount( "Admin_asyncEnabled", asyncEnabled, admin, 
# 587 "admin/Admin.c" 3 4
   0
# 587 "admin/Admin.c"
   , 
# 587 "admin/Admin.c" 3 4
...
# 588 "admin/Admin.c"
   , ((struct Admin_FunctionArg[]) { { .name = "page", .required = 0, .type = "Int" } }), (sizeof(((struct Admin_FunctionArg[]) { { .name = "page", .required = 0, .type = "Int" } })) / sizeof(struct Admin_FunctionArg)), &admin->pub)
                        ...
                        ;
    Admin_registerFunctionWithArgCount( "Admin_importFd", importFd, admin, 
# 592 "admin/Admin.c" 3 4
   1
# 592 "admin/Admin.c"
   , 
# 592 "admin/Admin.c" 3 4
...
# 592 "admin/Admin.c"
    ...
    Admin_registerFunctionWithArgCount( "Admin_exportFd", exportFd, admin, 
# 593 "admin/Admin.c" 3 4
   1
# 593 "admin/Admin.c"
   , ((struct Admin_FunctionArg[]) { { .name = "fd", .required = 1, .type = "Int" } }), (sizeof(((struct Admin_FunctionArg[]) { { .name = "fd", .required = 1, .type = "Int" } })) / sizeof(struct Admin_FunctionArg)), &admin->pub)
