...
# 1 "./util/Linker.h" 1
# 20 "./util/Assert.h" 2
struct Linker_x01877139549028477;
# 33 "./util/Assert.h"
__attribute__((__format__ (__printf__, 1, 2)))
...
struct Linker_x030051626525745867;
...
struct Linker_x030487575461883165;
...
# 1 "./util/Bits.h" 1
# 21 "./util/Bits.h"
struct Linker_x06480927134086891;
...
struct Linker_x0800532636323235;
...
struct Linker_x04069889488038059;
...
struct Linker_x022433639990331034;
...
struct Linker_x07395046864896375;
...
# 35 "interface/addressable/AddrIfaceAdapter.c"
       )); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0xe35a1df31cb00267ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("AddrIfaceAdapter.c"), (35), "(Identity_ptr->Identity_verifier == ((unsigned long) 0xe35a1df31cb00267ull))"); } } while (0); Identity_ptr; })); }));

    do {             struct Er_Ret* Er_ret = Message_eshift(msg, -(ctx->pub.generic.addr->addrLen));                          if (Er_ret) {                 struct Er_Ret** Er_errOut = (struct Er_Ret**)0;                 if (Er_errOut) {                     *Er_errOut = Er_unwind("AddrIfaceAdapter.c", 37, Er_ret);                 } else {                     Er__assertFail(Er_unwind("AddrIfaceAdapter.c", 37, Er_ret));                 }             }                  } while (0);
...
# 44 "interface/addressable/AddrIfaceAdapter.c"
       )); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0xe35a1df31cb00267ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("AddrIfaceAdapter.c"), (44), "(Identity_ptr->Identity_verifier == ((unsigned long) 0xe35a1df31cb00267ull))"); } } while (0); Identity_ptr; })); }));

    do {             struct Er_Ret* Er_ret = Message_epush(msg, ctx->pub.generic.addr, ctx->pub.generic.addr->addrLen);                          if (Er_ret) {                 struct Er_Ret** Er_errOut = (struct Er_Ret**)0;                 if (Er_errOut) {                     *Er_errOut = Er_unwind("AddrIfaceAdapter.c", 46, Er_ret);                 } else {                     Er__assertFail(Er_unwind("AddrIfaceAdapter.c", 46, Er_ret));                 }             }                  } while (0);
...
struct AddrIfaceAdapter* AddrIfaceAdapter_new(struct Allocator* alloc)
{
    ...
    (context)->Identity_verifier = ((unsigned long) 0xe35a1df31cb00267ull);
    return &context->pub;
}
