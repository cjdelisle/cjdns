# 1 "crypto/Sign_admin.c"
# 1 "/home/jojapoppa/cjdns//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "crypto/Sign_admin.c"
# 15 "crypto/Sign_admin.c"
# 1 "./crypto/Sign_admin.h" 1
# 18 "./crypto/Sign_admin.h"
# 1 "./admin/Admin.h" 1
# 18 "./admin/Admin.h"
...
# 1 "./util/Linker.h" 1
# 20 "./util/Assert.h" 2
struct Linker_x08680492051560402;
# 33 "./util/Assert.h"
__attribute__((__format__ (__printf__, 1, 2)))
...
struct Linker_x06656562352636795;
...
struct Linker_x004837456346607727;
...
# 1 "./util/Bits.h" 1
# 21 "./util/Bits.h"
struct Linker_x042860413894917304;
...
struct Linker_x09784818172242475;
...
struct Linker_x031051662735991337;
...
struct Linker_x05455692125110054;
...
struct Linker_x07027431167896436;
...
# 1 "./util/log/Log.h" 1
# 20 "./util/log/Log.h"
struct Linker_x06071510441153274;
...
struct Linker_x06045801966465238;
...
struct Linker_x03266739732220909;
...
struct Linker_x05667875519755228;
...
# 21 "./crypto/Sign_admin.h" 2

struct Linker_x039805726989961454;

void Sign_admin_register(uint8_t* privateKey,
                         struct Admin* admin,
                         struct Random* rand,
                         struct Allocator* alloc);
# 16 "crypto/Sign_admin.c" 2

# 1 "./admin/Admin.h" 1
# 18 "crypto/Sign_admin.c" 2
# 1 "./benc/String.h" 1
# 20 "./benc/String.h"
# 1 "./util/CString.h" 1
# 21 "./util/CString.h"
struct Linker_x08186216333726339;
...
struct Linker_x0535628452747525;
...
# 19 "crypto/Sign_admin.c" 2
# 1 "./crypto/Key.h" 1
# 22 "./crypto/Key.h"
struct Linker_x0032921870281082954;
...
# 20 "crypto/Sign_admin.c" 2
# 1 "./crypto/Sign.h" 1
# 21 "./crypto/Sign.h"
struct Linker_x06543236854935588;
...
# 21 "crypto/Sign_admin.c" 2
# 1 "./dht/Address.h" 1
# 21 "./dht/Address.h"
struct Linker_x05197780025836296;
# 34 "./dht/Address.h"
struct Address
{
...
{
    struct Context* ctx = (__extension__ ({ __typeof__((struct Context*) vctx) Identity_ptr = ((struct Context*) vctx); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0xd09b97c43cd6b835ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Sign_admin.c"), (39), "(Identity_ptr->Identity_verifier == ((unsigned long) 0xd09b97c43cd6b835ull))"); } } while (0); Identity_ptr; }));
    String* msgHash = Dict_getString(args, (&(String) { .bytes = "msgHash", .len = CString_strlen("msgHash") }));
    String* signature = Dict_getString(args, (&(String) { .bytes = "signature", .len = CString_strlen("signature") }));
    Dict* out = Dict_new(requestAlloc);
    uint8_t publicSigningKey[32];
    uint8_t sigBytes[64];
    char* underscore = CString_strchr(signature->bytes, '_');
    if (msgHash->len > 64) {
        Dict_putString(out, String_new("error", requestAlloc), String_new("msgHash too long, max 64 bytes", requestAlloc), requestAlloc);
    } else if (underscore == 
# 48 "crypto/Sign_admin.c" 3 4
                            ((void *)0)
# 48 "crypto/Sign_admin.c"
                                ) {
        Dict_putString(out, String_new("error", requestAlloc), String_new("malformed signature, missing separator", requestAlloc), requestAlloc)
                                                                   ;
    } else if (Base32_decode(
        publicSigningKey, 32, signature->bytes, (int)(underscore - signature->bytes)) != 32)
    {
        Dict_putString(out, String_new("error", requestAlloc), String_new("malformed signature, failed to decode pubkey", requestAlloc), requestAlloc)
                                                                         ;
    } else if (Base32_decode(
        sigBytes, 64, &underscore[1], CString_strlen(&underscore[1])) != 64)
    {
        Dict_putString(out, String_new("error", requestAlloc), String_new("malformed signature, failed to decode signature", requestAlloc), requestAlloc)
                                                                            ;
    } else {
        struct Message* msg = Message_new(0, msgHash->len + 64, requestAlloc);
        do {             struct Er_Ret* Er_ret = Message_epush(msg, msgHash->bytes, msgHash->len);                          if (Er_ret) {                 struct Er_Ret** Er_errOut = (struct Er_Ret**)0;                 if (Er_errOut) {                     *Er_errOut = Er_unwind("Sign_admin.c", 63, Er_ret);                 } else {                     Er__assertFail(Er_unwind("Sign_admin.c", 63, Er_ret));                 }             }                  } while (0);
        do {             struct Er_Ret* Er_ret = Message_epush(msg, sigBytes, 64);                          if (Er_ret) {                 struct Er_Ret** Er_errOut = (struct Er_Ret**)0;                 if (Er_errOut) {                     *Er_errOut = Er_unwind("Sign_admin.c", 64, Er_ret);                 } else {                     Er__assertFail(Er_unwind("Sign_admin.c", 64, Er_ret));                 }             }                  } while (0);
        uint8_t curve25519key[32];
        if (Sign_verifyMsg(publicSigningKey, msg)) {
            Dict_putString(out, String_new("error", requestAlloc), String_new("invalid signature", requestAlloc), requestAlloc);
        } else if (Sign_publicSigningKeyToCurve25519(curve25519key, publicSigningKey)) {
            Dict_putString(out, String_new("error", requestAlloc), String_new("not a valid curve25519 key", requestAlloc), requestAlloc);
        } else {
            struct Address addr;
            ...
            Dict_putString(out, String_new("pubkey", requestAlloc), k, requestAlloc);
            Dict_putString(out, String_new("ipv6", requestAlloc), String_new(ipv6, requestAlloc), requestAlloc);
...
{
    struct Context* ctx = (__extension__ ({ __typeof__((struct Context*) vctx) Identity_ptr = ((struct Context*) vctx); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0xd09b97c43cd6b835ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Sign_admin.c"), (86), "(Identity_ptr->Identity_verifier == ((unsigned long) 0xd09b97c43cd6b835ull))"); } } while (0); Identity_ptr; }));
    String* msgHash = Dict_getString(args, (&(String) { .bytes = "msgHash", .len = CString_strlen("msgHash") }));
    Dict* out = Dict_new(requestAlloc);
    if (msgHash->len > 64) {
        Dict_putString(out, String_new("error", requestAlloc), String_new("msgHash too long, max 64 bytes", requestAlloc), requestAlloc);
        ...
        struct Message* msg = Message_new(0, msgHash->len + 64, requestAlloc);
        do {             struct Er_Ret* Er_ret = Message_epush(msg, msgHash->bytes, msgHash->len);                          if (Er_ret) {                 struct Er_Ret** Er_errOut = (struct Er_Ret**)0;                 if (Er_errOut) {                     *Er_errOut = Er_unwind("Sign_admin.c", 93, Er_ret);                 } else {                     Er__assertFail(Er_unwind("Sign_admin.c", 93, Er_ret));                 }             }                  } while (0);
        Sign_signMsg(ctx->signingKeypair, msg, ctx->rand);
        uint8_t signB64[128];
        do { if (!((Base32_encode(signB64, 128, msg->bytes, 64) > 0))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Sign_admin.c"), (96), "(Base32_encode(signB64, 128, msg->bytes, 64) > 0)"); } } while (0);
        String* sig = String_printf(requestAlloc, "%s_%s", ctx->pubSigningKey, signB64);
        Dict_putString(out, String_new("signature", requestAlloc), sig, requestAlloc);
        Dict_putString(out, String_new("error", requestAlloc), String_new("none", requestAlloc), requestAlloc);
    }
    Admin_sendMessage(out, txid, ctx->admin);
}
...
void Sign_admin_register(uint8_t* privateKey,
                         ...
                         struct Allocator* alloc)
{
    struct Context* ctx = Allocator__calloc((alloc),(sizeof(struct Context)),(1),"Sign_admin.c",109);
    Sign_signingKeyPairFromCurve25519(ctx->signingKeypair, privateKey);
    uint8_t sPubKey[32];
    Sign_publicKeyFromKeyPair(sPubKey, ctx->signingKeypair);
    do { if (!((Base32_encode(ctx->pubSigningKey, 64, sPubKey, 32) > 0))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Sign_admin.c"), (113), "(Base32_encode(ctx->pubSigningKey, 64, sPubKey, 32) > 0)"); } } while (0);

    ctx->alloc = alloc;
    ctx->rand = rand;
    ctx->admin = admin;
    (ctx)->Identity_verifier = ((unsigned long) 0xd09b97c43cd6b835ull);

    Admin_registerFunctionWithArgCount( "Sign_checkSig", checkSig, ctx, 
# 120 "crypto/Sign_admin.c" 3 4
   0
# 120 "crypto/Sign_admin.c"
   , ((struct Admin_FunctionArg[]) { { .name = "msgHash", .required = 
# 120 "crypto/Sign_admin.c" 3 4
   1
# 120 "crypto/Sign_admin.c"
   , .type = "String" }, { .name = "signature", .required = 
# 120 "crypto/Sign_admin.c" 3 4
   1
# 120 "crypto/Sign_admin.c"
   , .type = "String" }, }), (sizeof(((struct Admin_FunctionArg[]) { { .name = "msgHash", .required = 
# 120 "crypto/Sign_admin.c" 3 4
   1
# 120 "crypto/Sign_admin.c"
   , .type = "String" }, { .name = "signature", .required = 
...
# 120 "crypto/Sign_admin.c"
   , .type = "String" }, })) / sizeof(struct Admin_FunctionArg)), admin)
                  ...
                  ;
    Admin_registerFunctionWithArgCount( "Sign_sign", sign, ctx, 
# 125 "crypto/Sign_admin.c" 3 4
   1
# 125 "crypto/Sign_admin.c"
   , ((struct Admin_FunctionArg[]) { { .name = "msgHash", .required = 
# 125 "crypto/Sign_admin.c" 3 4
   1
# 125 "crypto/Sign_admin.c"
